...
import {
  ...
  type SingingMelodyMode,
} from '@codex-im-suite/contracts/speech';

export const SPEECH_PREVIEW_CONTROL_PROTOCOL = 'cti-speech-preview-control/v4' as const;
...
// 该上限只约束受限本机 mailbox。试听回执不含路径；参考文件路径仅存在于
// 短期请求文件，响应只返回 Hash 与匹配状态，不放宽媒体或文本门禁。
...
export interface SpeechPreviewControlRequest {
  ...
  requestId: string;
  clientNonce: string;
  action: 'preview_voice' | 'benchmark_voice' | 'preview_singing_voice' | 'benchmark_singing_voice' | 'generate_singing' | 'verify_reference_transcript';
  requestedAt: string;
  expiresAt: string;
  input: {
    text?: string;
...
export interface SpeechPreviewControlResponse {
  ...
  clientNonce: string;
  ok: boolean;
  result?: SpeechPreviewReceipt | RuntimeReferenceTranscriptVerificationReceipt;
  errorCode?: string;
  status?: Exclude<SpeechRuntimeState, 'ready'>;
  error?: string;
  /** 失败观察仅允许数值质量指标，禁止把歌词、转写、路径或身份跨进程传递。 */
  qualityMetrics?: {
...
function validateRequest(
  ...
  if (
    ...
    || (request.action !== 'preview_voice' && request.action !== 'benchmark_voice'
      ...
      && request.action !== 'generate_singing'
      && request.action !== 'verify_reference_transcript')
    ...
    || !request.input
    || (request.action === 'verify_reference_transcript'
      ? (!safeReferenceSourcePath(request.input.sourcePath)
        || !safeConfirmedTranscript(request.input.confirmedTranscript)
        || Object.keys(request.input).some((key) => !['sourcePath', 'confirmedTranscript'].includes(key)))
      : request.action === 'preview_singing_voice' || request.action === 'benchmark_singing_voice' || request.action === 'generate_singing'
        ? !singingPlanFromRequest(
            request.input,
            request.action === 'generate_singing' ? 'full_generation' : 'quick_preview',
          )
      : (!safePreviewText(request.input.text)
        || !safeModelId(request.input.modelId)
        || !safeVoiceProfileId(request.input.voiceProfileId)
        || Object.keys(request.input).some((key) => !['text', 'modelId', 'voiceProfileId'].includes(key))))
  ) {
...
function singingPlanFromRequest(
  input: SpeechPreviewControlRequest['input'],
  outputMode: 'quick_preview' | 'full_generation',
): SingingAudioContentPlanContract | null {
  if (Object.keys(input).some((key) => ![
    'lyrics', 'stylePrompt', 'vocalLanguage', 'durationSeconds', 'melodyMode',
    'modelId', 'voiceProfileId',
  ].includes(key)) || !safeModelId(input.modelId) || !safeVoiceProfileId(input.voiceProfileId)) return null;
  const plan = createSingingAudioContentPlan({
    ...
    melodyMode: input.melodyMode,
    maxLyricsCharacters: 6_000,
    maxDurationSeconds: 600,
    ...(input.voiceProfileId !== 'acestep.default' ? { voiceRequirement: 'active_reference' as const } : {}),
  });
  return plan
    && plan.lyrics === input.lyrics
    && plan.stylePrompt === input.stylePrompt
    && plan.vocalLanguage === input.vocalLanguage
    && plan.durationSeconds === input.durationSeconds
    && plan.melodyMode === input.melodyMode
    ? plan
    : null;
}

function validateReferenceTranscriptVerificationReceipt(
  value: unknown,
): RuntimeReferenceTranscriptVerificationReceipt {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new RuntimeSpeechError('speech_reference_transcript_response_invalid', 'error', '参考文本核对响应无效');
  }
  const receipt = value as Partial<RuntimeReferenceTranscriptVerificationReceipt>;
  if (Object.keys(receipt).some((key) => !['protocol', 'sourceSha256', 'transcriptStatus', 'validated'].includes(key))
    || receipt.protocol !== 'cti-speech-reference-transcript-verification/v1'
    || !/^[a-f0-9]{64}$/u.test(receipt.sourceSha256 || '')
    || receipt.transcriptStatus !== 'matched'
...
function validatePreviewReceipt(
  ...
  const requiredKeys = [
    ...
    ...(singing ? ['lyricsAlignmentStatus', 'lyricsAlignment', 'lyricsAlignmentThreshold', 'lyricsAlignmentPassed'] : []),
  ...
  if (
    ...
    || requiredKeys.some((key) => !Object.prototype.hasOwnProperty.call(receipt, key))
    ||
    ...
    || (receipt.speakerSimilarityStatus === 'passed' && (!similarityFieldsPresent
      ...
      || receipt.speakerSimilarityPassed !== (receipt.speakerSimilarity >= receipt.speakerSimilarityThreshold)))
    || ((receipt.speakerSimilarityStatus === 'not_applicable' || receipt.speakerSimilarityStatus === 'not_verified') && similarityFieldsPresent)
    || (singing && (receipt.lyricsAlignmentStatus !== 'passed'
      || typeof receipt.lyricsAlignment !== 'number'
      || !Number.isFinite(receipt.lyricsAlignment)
      || receipt.lyricsAlignment < 0 || receipt.lyricsAlignment > 1
      || typeof receipt.lyricsAlignmentThreshold !== 'number'
      || !Number.isFinite(receipt.lyricsAlignmentThreshold)
      || receipt.lyricsAlignmentThreshold < 0 || receipt.lyricsAlignmentThreshold > 1
  ...
  return {
    ...
    speakerSimilarityStatus: receipt.speakerSimilarityStatus,
    ...(receipt.speakerSimilarityStatus === 'passed' ? {
      speakerSimilarity: receipt.speakerSimilarity!,
      speakerSimilarityThreshold: receipt.speakerSimilarityThreshold!,
      speakerSimilarityPassed: true,
    } : {}),
    ...(singing ? {
      lyricsAlignmentStatus: 'passed' as const,
      lyricsAlignment: receipt.lyricsAlignment!,
      lyricsAlignmentThreshold: receipt.lyricsAlignmentThreshold!,
      lyricsAlignmentPassed: true,
    } : {}),
    ...(benchmark ? {
      modelRevision: receipt.modelRevision!,
      peakVramMiB: receipt.peakVramMiB!,
    } : {}),
    validated: true,
  };
}

function safeReferenceSourcePath(value: unknown): value is string {
  return typeof value === 'string'
    && value.trim() === value
    && value.length > 0
    && value.length <= 4_000
...
function safeConfirmedTranscript(value: unknown): value is string {
  return typeof value === 'string'
    && value.trim() === value
    && value.length > 0
      ...
      for (const name of names) {
        ...
        try {
          ...
          try {
            ...
            expiryTimer.unref?.();
            try {
              let result: SpeechPreviewReceipt | RuntimeReferenceTranscriptVerificationReceipt;
              if (request.action === 'verify_reference_transcript') {
                if (!options.verifyReferenceTranscript) {
                  throw new RuntimeSpeechError('speech_reference_transcript_live_runtime_unavailable', 'blocked', '实时参考文本核对通道不可用');
                }
                result = validateReferenceTranscriptVerificationReceipt(await options.verifyReferenceTranscript({
                  sourcePath: request.input.sourcePath!,
                  confirmedTranscript: request.input.confirmedTranscript!,
                  signal: currentAbort.signal,
                }));
              } else {
                const singingAction = request.action === 'preview_singing_voice'
                ...
                result = singingAction
                  ? await (preview as NonNullable<typeof options.previewSingingVoice>)({
                      plan: singingPlanFromRequest(
                        request.input,
                        request.action === 'generate_singing' ? 'full_generation' : 'quick_preview',
                      )!,
                      modelId: request.input.modelId!,
                      voiceProfileId: request.input.voiceProfileId!,
                      signal: currentAbort.signal,
                    })
                  : await (preview as typeof options.previewVoice)({
              ...
              if (!stopped && now().getTime() < expiresAt) {
                outgoing = {
                  ...
                  ok: true,
                  result: request.action === 'verify_reference_transcript'
                    ? validateReferenceTranscriptVerificationReceipt(result)
                    : validatePreviewReceipt(
                      ...
                      request.action === 'preview_singing_voice'
                        || request.action === 'benchmark_singing_voice'
                        || request.action === 'generate_singing',
                    ),
            ...
            if (
              ...
              outgoing = {
                ...
                ...(failure.qualityMetrics ? { qualityMetrics: failure.qualityMetrics } : {}),
                respondedAt: now().toISOString(),
              };
            }
          }
...
async function requestControl(input: {
  runtimeStateRoot: string;
  requestInput: SpeechPreviewControlRequest['input'];
  timeoutMs: number;
  action: SpeechPreviewControlRequest['action'];
  now?: () => Date;
}): Promise<SpeechPreviewReceipt | RuntimeReferenceTranscriptVerificationReceipt> {
  ...
  validateRequest(request, requestId, requestedAt.getTime());
  ...
  try {
      ...
      if (fs.existsSync(responsePath)) {
        ...
        if (!response.ok) {
          const code = typeof response.errorCode === 'string' && /^[a-z0-9_]{1,80}$/iu.test(response.errorCode)
            ? response.errorCode
            : input.action === 'verify_reference_transcript'
              ? 'speech_reference_transcript_verification_failed'
          ...
          throw new RuntimeSpeechError(
            ...
            typeof response.error === 'string' && response.error.length <= 200
              ? response.error
              : input.action === 'verify_reference_transcript' ? '参考文本核对失败' : '语音试听失败',
            response.qualityMetrics && typeof response.qualityMetrics === 'object'
              && !Array.isArray(response.qualityMetrics)
              ? {
                  ...(typeof response.qualityMetrics.lyricsAlignment === 'number'
                    && Number.isFinite(response.qualityMetrics.lyricsAlignment)
                    && response.qualityMetrics.lyricsAlignment >= 0 && response.qualityMetrics.lyricsAlignment <= 1
                    ? { lyricsAlignment: response.qualityMetrics.lyricsAlignment } : {}),
                  ...(typeof response.qualityMetrics.lyricsAlignmentThreshold === 'number'
                    && Number.isFinite(response.qualityMetrics.lyricsAlignmentThreshold)
                    && response.qualityMetrics.lyricsAlignmentThreshold >= 0 && response.qualityMetrics.lyricsAlignmentThreshold <= 1
                    ? { lyricsAlignmentThreshold: response.qualityMetrics.lyricsAlignmentThreshold } : {}),
                  ...(response.qualityMetrics.lyricsAlignmentPassed === false
                    ? { lyricsAlignmentPassed: false as const } : {}),
                }
              : undefined,
          );
        }
        return input.action === 'verify_reference_transcript'
          ? validateReferenceTranscriptVerificationReceipt(response.result)
          : validatePreviewReceipt(
            response.result,
            input.requestInput.modelId!,
            input.requestInput.voiceProfileId!,
            input.action === 'benchmark_voice' || input.action === 'benchmark_singing_voice',
            input.action === 'preview_singing_voice'
              || input.action === 'benchmark_singing_voice'
              || input.action === 'generate_singing',
          );
      }
    ...
    throw new RuntimeSpeechError(
      input.action === 'verify_reference_transcript'
        ? 'speech_reference_transcript_live_timeout'
        : 'speech_preview_live_timeout',
      'error',
      input.action === 'verify_reference_transcript'
        ? '实时 Bridge 未在限定时间内完成参考文本核对'
...
async function requestPreview(input: {
  ...
  voiceProfileId: string;
  timeoutMs: number;
  action: 'preview_voice' | 'benchmark_voice';
  now?: () => Date;
}): Promise<SpeechPreviewReceipt> {
  ...
  return requestControl({
    ...
    requestInput: { text: input.text, modelId: input.modelId, voiceProfileId: input.voiceProfileId },
    timeoutMs: input.timeoutMs,
    action: input.action,
    now: input.now,
  }) as Promise<SpeechPreviewReceipt>;
}

async function requestSinging(input: {
  runtimeStateRoot: string;
  plan: SingingAudioContentPlanContract;
  modelId: string;
  voiceProfileId: string;
  timeoutMs: number;
  action: 'preview_singing_voice' | 'benchmark_singing_voice' | 'generate_singing';
  now?: () => Date;
}): Promise<SpeechPreviewReceipt> {
  if (!safeModelId(input.modelId) || !safeVoiceProfileId(input.voiceProfileId)) {
    throw new RuntimeSpeechError('singing_preview_request_invalid', 'blocked', '歌声生成参数无效');
  }
  return requestControl({
    runtimeStateRoot: input.runtimeStateRoot,
    requestInput: {
      lyrics: input.plan.lyrics,
      stylePrompt: input.plan.stylePrompt,
      vocalLanguage: input.plan.vocalLanguage,
      durationSeconds: input.plan.durationSeconds,
      melodyMode: input.plan.melodyMode,
...
export function requestReferenceTranscriptVerification(input: {
  runtimeStateRoot: string;
  sourcePath: string;
  confirmedTranscript: string;
  timeoutMs: number;
  ...
  if (!safeReferenceSourcePath(input.sourcePath) || !safeConfirmedTranscript(input.confirmedTranscript)) {
    throw new RuntimeSpeechError('speech_reference_transcript_request_invalid', 'blocked', '参考文本核对参数无效');
  }
  return requestControl({
    runtimeStateRoot: input.runtimeStateRoot,
    requestInput: { sourcePath: input.sourcePath, confirmedTranscript: input.confirmedTranscript },
    timeoutMs: input.timeoutMs,
    action: 'verify_reference_transcript',
    now: input.now,
  }) as Promise<RuntimeReferenceTranscriptVerificationReceipt>;
}
...
export function requestSingingVoicePreview(input: {
  ...
  return requestSinging({ ...input, action: 'preview_singing_voice' });
}
...
export function requestSingingVoiceBenchmark(input: {
  ...
  return requestSinging({ ...input, action: 'benchmark_singing_voice' });
}

export function requestSingingGeneration(input: {
  runtimeStateRoot: string;
  plan: SingingAudioContentPlanContract;
  modelId: string;
  voiceProfileId: string;
  ...
  return requestSinging({ ...input, action: 'generate_singing' });
}
