...
export function loadSpeechRuntimeConfig(env: ReadonlyMap<string, string>): SpeechRuntimeConfig {
  ...
  return {
    ...
    requireVoiceSimilarityAcceptance: bool(env, 'CTI_SPEECH_REQUIRE_VOICE_SIMILARITY_ACCEPTANCE', false),
    ownerSelfVoiceAutoAuthorization: bool(env, 'CTI_SPEECH_OWNER_SELF_VOICE_AUTO_AUTHORIZATION', false),
    ...
    singingLyricsVerifier: id(env, 'CTI_SINGING_LYRICS_VERIFIER', 'sensevoice_gguf'),
    singingLyricsVerifierWslDistro: optional(env, 'CTI_SINGING_LYRICS_VERIFIER_WSL_DISTRO'),
    singingLyricsVerifierWslPython: optional(env, 'CTI_SINGING_LYRICS_VERIFIER_WSL_PYTHON'),
    singingLyricsVerifierScriptPath: optional(env, 'CTI_SINGING_LYRICS_VERIFIER_SCRIPT_PATH'),
    singingLyricsVerifierModelPath: optional(env, 'CTI_SINGING_LYRICS_VERIFIER_MODEL_PATH'),
    ...
    synthesisTimeoutMs: boundedInt(env, 'CTI_SPEECH_SYNTHESIS_TIMEOUT_MS', 600_000, 1_000, 600_000),
...
export function speechConfigToEnvEntries(config: SpeechRuntimeConfig): Array<[string, string]> {
  return [
    ...
    ['CTI_SPEECH_REQUIRE_VOICE_SIMILARITY_ACCEPTANCE', String(config.requireVoiceSimilarityAcceptance)],
    ['CTI_SPEECH_OWNER_SELF_VOICE_AUTO_AUTHORIZATION', String(config.ownerSelfVoiceAutoAuthorization)],
    ...
    ['CTI_SINGING_LYRICS_VERIFIER', config.singingLyricsVerifier],
    ['CTI_SINGING_LYRICS_VERIFIER_WSL_DISTRO', config.singingLyricsVerifierWslDistro || ''],
    ['CTI_SINGING_LYRICS_VERIFIER_WSL_PYTHON', config.singingLyricsVerifierWslPython || ''],
    ['CTI_SINGING_LYRICS_VERIFIER_SCRIPT_PATH', config.singingLyricsVerifierScriptPath || ''],
    ['CTI_SINGING_LYRICS_VERIFIER_MODEL_PATH', config.singingLyricsVerifierModelPath || ''],
    ...
    ['CTI_SPEECH_SYNTHESIS_TIMEOUT_MS', String(config.synthesisTimeoutMs)],
