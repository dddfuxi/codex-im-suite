...
- **可选本地语音与歌声预览** — 开发版 `0.4.0` 新增 Qwen3-TTS 模型/音色/语气选择、模型级 benchmark、飞书首发的本地 ASR/TTS、独立 ACE-Step 歌声和原生 Opus 回复；能力默认关闭
- **会话持久化** — 对话在守护进程重启后保留
...
默认语音模型为 Qwen3-TTS 12Hz 1.7B CustomVoice + Serena；0.6B CustomVoice 是显式低显存选项，1.7B/0.6B Base 只用于已授权参考音色复刻。自然语言语音意图由 Primary 输出受限结构，再由 Bridge 结合真实 evidence、会话状态、角色与风险裁决，禁止用关键词正则冒充授权或完成门禁。
