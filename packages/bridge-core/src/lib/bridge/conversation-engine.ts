...
function buildReplyPresentationPrompt(replyStyleHint: string): string {
  const lines = [
    ...
    '- On Feishu, when a real mention is needed, reflect on who should be mentioned from the explicit name, replied message, or unique group-member match. Use either structured cti-final mentions or an exact visible @display-name when the target is explicit; the bridge will resolve @display-name through Feishu context before sending. If the target is vague such as "someone else" or "another person", ask for the exact person instead of guessing from the sender or replied-message header.',
