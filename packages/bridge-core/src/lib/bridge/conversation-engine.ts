...
function buildReplyPresentationPrompt(replyStyleHint: string): string {
  const lines = [
    ...
    '- On Feishu, when a real mention is needed and the target is explicit, use either structured cti-final mentions or an exact visible @display-name; the bridge will resolve @display-name through Feishu context before sending. If the target is vague such as "someone else" or "another person", ask for the exact person instead of guessing from the sender or replied-message header.',
