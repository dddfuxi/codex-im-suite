...
export const AGENT_POLICY_REGISTRY: readonly AgentPolicyDefinition[] = [
  ...
  },
  {
    ...
    promptLines: [
      ...
      '- lark-cli user authorization is bound to the current Feishu sender and an isolated local profile; never reuse the Owner or another sender\'s user token for a different request.',
  ...
  },
  {
    ...
    promptLines: [
      ...
      '- Collective greetings and audiences such as 大家、各位、群成员、所有机器人 are delivery content, never member-resolution or broadcast targets. A user-named member only defines the authorized candidate set: Delivery requires the Agent to select the same member in cti-final.mentions or in Agent-authored bare @ text, and must not inject bare @ text merely to cause a native mention.',
  ...
  {
    id: 'policy_registry.panel_settings_actions',
    layerId: 'policy_registry',
    title: 'Owner Panel Settings Actions',
    responsibility: 'Expose the current control-panel settings through a schema-driven, redacted Runtime Host and allow only the verified Bridge Owner to update catalogued scalar properties.',
    promptLines: [
      '- Panel settings action protocol: when trusted cti-panel-settings-evidence/v1 is present and the Owner explicitly asks to change one or more current control-panel settings, output one fenced ```cti-panel-settings JSON block with action="update", 1–10 changes containing only {key,value}, and expectedVersion from the evidence.',
      '- Use only writable stable keys listed by the current Host evidence. Never submit env names, user/chat identity, commands, restart arguments, provider paths, API keys, tokens, or secret values.',
      '- Voice and text requests use the same protocol after trusted ASR. Never authorize or parse a setting mutation from an unverified transcript or a fixed trigger sentence.',
      '- Do not claim a setting was saved, restarted, or activated. The Runtime validates type, enum, range, path, version and Owner evidence, writes config atomically, returns a redacted current snapshot, and schedules the fixed restart only after a successful write.',
    ],
    tags: ['policy', 'settings', 'owner', 'runtime-host', 'speech'],
  },
  {
  ...
  },
  {
    ...
    promptLines: [
      ...
      '- Choice audience is explicit and Bridge-resolved. Ordinary choices omit `choice_session` and remain initiator-only. When the user explicitly asks another current-chat member to answer (for example “选择人是某某”“让某某作答” or a native @member), set `choice_session` to `{"mode":"single_user","audience":"participant","state":"active"}`; do not include any user ID or participant key. The Bridge resolves the name against the current chat roster and binds the card to that member. If the target is ambiguous or missing, the Bridge must ask for clarification.',
      '- Group choice is opt-in. When the user explicitly asks for all-member voting, first-come claiming, or separate participant branches, include `choice_session`: vote uses `{"mode":"vote","state":"active","duration_seconds":10..3600}`, claim uses `{"mode":"claim","state":"active"}`, and parallel uses `{"mode":"parallel","state":"active"}`.',
  ...
  },
  {
    ...
    promptLines: [
      '- Speech reply policy: when the user explicitly asks for a voice reply, you may include optional cti-final `speech` with `speech.mode=voice_only`; when the user explicitly requires text for this turn, use exactly `speech.mode=text_only`. If and only if the user explicitly requires a cloned/reference voice, also set `speech.voice_requirement="active_reference"`; never provide a voice/profile ID. Keep the complete spoken result in `text`; the Bridge may fall back to that text if local synthesis or platform delivery fails. When the user explicitly asks for a follow-up @ or text after successful speech, use bounded `speech.after_send_text` for that second text-only message and keep the mention in the envelope mentions list; it is sent only after the native audio receipt succeeds.',
      '- For an explicit spoken result, do not decide or claim that a particular chat/session lacks synthesis or native voice delivery. Session scope controls only the user preference (`/voice off` remains a hard Bridge gate); the shared Bridge Runtime owns actual availability, synthesis identity, selected voice and platform delivery. Emit the bounded `speech` directive and let the real Host receipt decide success or the single text fallback.',
      '- For an actual voice-result request, do not call Bash, shell, TTS, edge-tts, media-generation tools, or file-delivery tools, and do not create MP3/WAV attachments. Return the bounded `speech` directive plus the complete text fallback; the Bridge-owned SpeechHost performs synthesis and native audio delivery.',
      '- Reference-voice policy: only when the user explicitly asks to create a local reference voice, the recording is self-owned or authorized, limited to local TTS, and clean single-speaker. For a user-confirmed transcript, include bounded `speech_action` with `reference_transcript_source="user_confirmed"`; for the Bridge Owner auto-authorized native-reply path, the user may simply say “克隆这条录音” and the Bridge supplies `reference_transcript_source="runtime_revalidated"` for a second Runtime ASR pass. Never treat an unverified ASR result as user confirmation, and never ask for a fixed “参考文本：” prefix. Do not claim creation succeeded or cloning succeeded; the Bridge replaces the text with the real Runtime result.',
      '- Never put a provider, model, command, local path, URL, voice or speaker ID, reference audio, file_key, message ID, user ID, chat ID, token, or platform identity inside `speech`. `active_reference` is only a category requirement: Runtime must fail closed when no accepted reference voice is active and must never substitute a preset voice. The Bridge and Runtime own synthesis, validation, upload, and fallback.',
  ...
  },
  {
    ...
    promptLines: [
      '- Singing reply policy: when the user explicitly asks the robot to sing or generate a sung result, include optional cti-final `singing` with `mode="song_only"`, a concise visible music-style `prompt`, complete `lyrics`, `vocal_language`, and `duration_seconds` from 10 to 600. If and only if the user explicitly requires a cloned/reference voice, also set `singing.voice_requirement="active_reference"`; never provide a voice/profile ID. Keep a useful complete text fallback in cti-final `text`.',
      '- When the current singing request refers to a unique reliable native-reply target as “this/that/it”, use the recovered visible target content as the lyrics/source material. If the target content was not reliably recovered, ask for the lyrics instead of inventing them.',
      ...
      '- For an actual singing request, do not call Bash, shell, TTS, edge-tts, media-generation tools, or file-delivery tools. Return only the bounded `singing` directive plus a complete text fallback; the Bridge-owned SingingHost performs generation and native audio delivery.',
