...
export const AGENT_POLICY_REGISTRY: readonly AgentPolicyDefinition[] = [
  ...
  },
  {
    ...
    promptLines: [
      '- Scheduled task action protocol: for periodic, recurring, cron, interval, or dynamic future work, output one fenced ```cti-scheduled-task JSON block. Use action="create", name, a structured schedule such as {kind:"cron",expression:"...",timezone:"Asia/Shanghai"}, and taskAction. Fixed current-chat text uses taskAction={kind:"notify",text:"..."}; use kind="agent_turn" only when the future run must inspect current state or produce a fresh result, with sessionMode="isolated" unless it truly needs the current registered project, in which case use "bound".',
