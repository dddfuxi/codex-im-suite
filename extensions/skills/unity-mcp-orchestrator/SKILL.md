...
- Ordinary screenshot requests should use portrait Game view or the requested camera.
- Only explicit overview, horizontal, panorama, or whole-scene-layout requests should use a landscape 16:9 overview.
- Do not report a Scene View crop or arbitrary editor viewport as success when the user requested Game view, PreviewCamera, or a named camera.
- For Timeline scenes, evaluate the PlayableDirector at the requested time before rendering. A blank frame is a failed capture and must be retried or reported as the exact blocker.
...
### Timeline Scene Screenshot Constraint

When capturing a Unity Timeline scene, do not screenshot immediately after loading the scene. First identify the intended `PlayableDirector`, activate the correct Timeline object, set the director time to the requested frame (usually `0` for the first frame), call `Evaluate()`, then render from the bound camera at the requested aspect ratio. If the resulting image is blank, mostly one color, the wrong viewport, or a Scene View substitute for a requested camera, treat it as a failed capture and retry the exact Timeline/camera path.
