...
- 通用轻聊分流与模型用量计量：现有轻聊门禁之后可通过 `CTI_LIGHT_CHAT_ROUTER_PROVIDER=coordinator|jev` 和 `CTI_LIGHT_CHAT_ROUTER_MODE=off|shadow|assist` 选择分流 Provider。Jev 只用 Decisions API `choice` 判断 `light_chat / task / ambiguous`，不生成用户回复或按钮；命令、附件、工具、文档、仓库、权限和高风险请求继续直接进入 Primary。`shadow` 只记录对照结果，`assist` 仅接受最高概率 `≥0.80` 且领先第二名 `≥0.15` 的结果，高置信 `task` 跳过 Coordinator，其他情况交给 Coordinator，Jev 失败或低置信则安全回退。默认仍为 `coordinator + off`，与 `/jev` 普通判断/纯模式相互独立。
- Runtime 提供通用 `UsageMeter` 和 `cti-model-usage/v1` 合约，统一保存 Jev、Coordinator、Primary 及未来 Provider 的 Token、缓存 Token、延迟、费率、路由和回退字段到 `CTI_HOME\runtime\model-usage.json`。Provider 回报费用优先，本地带生效时间的价格表兜底；费率未知显示“未知”，不伪造为零。账本不保存完整 Prompt、回复、密钥或附件，控制面板显示脱敏明细、Provider/模型/日期汇总、失败率、p50/p95、Jev/Coordinator 一致率和 Jev 跳过 Coordinator 次数，普通飞书回复不携带用量信息。
...
- 模型选择器会由 Runtime 自动读取当前来源的可用目录：官方 Codex 使用 app-server `model/list`，OpenAI-compatible 使用 `/v1/models`，Ollama 使用 `/api/tags`；设置页对官方、本地和外部来源显示下拉选项，目录失败时保留默认模型或已有候选，不要求手动抄写模型名。
...
- 轻聊分流配置：`CTI_LIGHT_CHAT_ROUTER_PROVIDER` 与 `CTI_LIGHT_CHAT_ROUTER_MODE` 默认分别为 `coordinator`、`off`；控制面板“设置 → 结构化判断 / 轻聊分流”可选 Coordinator 或 Jev，以及 Off、Shadow、Assist，保存后需受控重启 Bridge。Jev assist 的固定门槛是最高概率 `0.80`、概率差距 `0.15`，不开放为任意用户输入。
...
- Bridge 对跨聊天的模型/工具回合默认限制为 2 个并发槽，避免多个 Codex、语音或工具进程同时把整机内存和显存推满；可用 `CTI_BRIDGE_MAX_CONCURRENT_TURNS` 调整到 1–8。官方/外部 Codex 单轮默认 15 分钟硬上限（`CTI_CODEX_TURN_TIMEOUT_MS` 可调，最多 30 分钟），本地 Codex CLI 默认 5 分钟；旧配置中的 `CTI_BRIDGE_PROCESSING_TIMEOUT_MS=0` 不再代表无限等待，而是回到安全默认值。
...
- 每轮回复都会记录执行证据；如果模型声称已生成图片、创建文件、执行命令或完成 Unity/MCP 当前状态检查，但没有成功工具记录，或 `cti-final` 声明的本地文件路径不存在，bridge 会在发送前拦截可能的假完成。对无工具、无权限请求、尚未发生 Bridge 动作的可恢复协议错误，Conversation Engine 会把稳定错误码、原任务和上一版结果交回同源模型，自动做一次禁工具的完整重写；仍失败才输出单一“未完成”。出现过任何工具或权限流程时不自动重放，Owner、身份、授权和高风险门禁也不会被该机制绕过。若 provider 没有返回任何可展示最终文本，Feishu 最终卡片会明确显示“未完成：模型没有返回可展示结果。”，不会只留下空白完成状态。
...
- 自然语言计划任务不走关键词最终回复：Agent 必须区分读取、固定通知、逐轮打卡、动态 Agent turn 和受控工具。明确“列出 / 查看 / 查询计划任务”时，Bridge 先按当前真实用户调用统一 Scheduled Task Host，把任务与同源状态投影成无路径、无动作正文、无平台身份的只读 evidence，再由 Agent 整理回答；Host 失败与“任务为零”严格区分。周期创建输出 `cti-scheduled-task`；单次低风险提醒可输出 `cti-reminder` 或使用 `/remind`，随后由统一 Host 创建。没有 Host success 时不能声称已创建，Codex 也不能自行写 Windows 计划任务、伪造打卡账本或直接调用飞书 API 伪装完成。
