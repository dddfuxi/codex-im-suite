...
- Runtime 提供通用 `UsageMeter` 和 `cti-model-usage/v1` 合约，统一保存 Jev、Coordinator、Primary 及未来 Provider 的 Token、缓存 Token、延迟、费率、路由和回退字段到 `CTI_HOME\runtime\model-usage.json`。Provider 回报费用优先，本地带生效时间的价格表兜底；费率未知显示“未知”，不伪造为零。账本不保存完整 Prompt、回复、密钥或附件，控制面板当前显示脱敏明细、Provider 汇总与 p50/p95，明细保留模型和日期；按模型/日期汇总与一致率列入本增量后续验收，普通飞书回复不携带用量信息。
