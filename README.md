...
- 人类可读投影：机器状态发生受控 mutation 时，面向人审核的既有 Markdown 入口必须在同一写锁事务中更新；任一投影失败则回滚机器状态。机器状态始终是唯一事实源，Markdown 只展示索引、状态、范围和可操作入口，受控区块之外的用户手写内容原样保留。记忆生命周期、表情包语义进化以及回合 Upload/Artifact/Scratch 清单均遵守此规则。
