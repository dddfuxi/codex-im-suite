...
- Feishu 出站 mention 的成员 payload 归一化、可 mention 用户 ID 过滤、候选按 ID 合并、证据优先级、同级歧义保留、history `<at>` 提取、DM receive ID 类型和原生标签构造统一由 `channels/feishu/mentions/outbound-mention-resolution.ts` 维护。adapter 只负责从本轮原生 mention、当前 sender、受控历史索引和当前群成员 API 收集真实候选，并执行实际消息发送；禁止把 app_id/bot_id、模型生成 ID、显示名猜测或 reply metadata 直接提升为可信 mention。
