...
- Feishu CardKit v2/v1 能力探测、完整接口门禁和 create/stream/settings/final update 请求差异统一由 `channels/feishu/cards/cardkit-compat.ts` 维护；active card、创建中 promise、初始状态与 throttle/typewriter 双计时器清理由 `streaming-card-registry.ts` 统一管理。adapter 只传入当前 REST Client 并编排真实 IM 消息发送、节流/打字机内容推进、最终展示与出站引用持久化；adapter stop、单卡结束和异常清理必须复用同一 registry 生命周期，禁止重新维护平行 Map 或遗漏任一计时器。
