...
- Feishu 表情包的可信来源/置信度门禁、具体语义判断、中文口语 n-gram、元描述停用词、`avoidWhen`、同群/使用次数/最近使用排序、精确 file key 与别名解析，以及 80 条状态库 retention 去重统一由 `channels/feishu/stickers/sticker-selection-policy.ts` 维护。当前时间和媒体缓存存在性必须由 adapter 显式注入；策略模块不得读取文件系统、状态库路径或调用平台发送。
