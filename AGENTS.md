...
- Feishu 云历史的增量/全量分页、水位停止、成员名映射、删除/system/空消息过滤、sticker 采集时序和本地索引 upsert 统一由 `channels/feishu/history/indexed-history-sync.ts` 编排；索引检索结果到 Provider prompt 的空结果、文档、定向引用和普通总结规则统一由 `indexed-history-prompt.ts` 构造。adapter 只注入平台分页、成员查询、正文解析、sticker 采集与索引查询能力；history 模块不得自行持有飞书凭据、恢复附件、调用 Provider 或把本地历史索引提升为长期记忆事实源。
