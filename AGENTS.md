...
- 结构化判断协议以 `packages/contracts/src/decision.ts` 与 `schemas/decision.schema.json` 为唯一共享来源；Core 只能通过可选、只读的 `DecisionProviderHost` 提交已裁决问题，Runtime 的 Jev 适配器只能调用专用 Decisions API，不得混入普通聊天 Provider、工具或发送链。`off / shadow / assist` 与 `off / explicit / auto` 的配置枚举必须保持一致；关闭、缺密钥、超时、取消或非法结果均失败关闭，显式或纯模式请求报告不可用，普通请求保留 Primary。Feishu Decision Card 只显示 `noul / choice / score` 和概率分布，不得生成 `cti-final.choices` 回调；新增替代 Provider、字段或展示入口时必须同步 Schema、定向测试、README、架构文档、开发日志和实施计划。
