...
- Feishu 云历史的增量/全量分页、水位停止、成员名映射、删除/system/空消息过滤、sticker 采集时序和本地索引 upsert 统一由 `channels/feishu/history/indexed-history-sync.ts` 编排；索引检索结果到 Provider prompt 的空结果、文档、定向引用和普通总结规则统一由 `indexed-history-prompt.ts` 构造；light context 的当前消息时间截断、reply 优先、self/system/删除过滤、短回复扩窗和“可能关联上文”锚点统一由 `light-context-selection.ts` 裁决；被回复历史消息的 image/sticker/file/audio/video/media/post/interactive 资源类型、精确 messageId/fileKey 绑定、顺序和去重由 `attachment-recovery.ts` 生成纯下载计划。adapter 只注入平台分页、成员查询、正文解析、sticker 采集、身份判断与索引查询能力，并负责真实 SDK/HTTP 下载、大小门禁、失败审计和附件注入；history 模块不得自行持有飞书凭据、执行网络下载、调用 Provider 或把本地历史索引提升为长期记忆事实源。
