...
- 轻量会话协调器与多 Agent 协作 Coordinator 必须保持分层：前者属于 Provider 路由层，只在确定性硬门禁后的短消息上输出严格 `reply / delegate / clarify`，不得携带工作区、附件、工具或权限，也不得直接发送；后者只负责复杂任务的 Specialist 规划。manual official/external 下轻量协调器必须复用同一已选模型来源的常驻 Codex app-server、后台预热并按会话隔离 ephemeral thread，禁止退回每轮新建 `codex exec` 或混入本地辅助模型；auto failover 和 `local_api` 继续服从原选择链。明确任务直接进入 Primary，轻量协调器超时、低置信、工具尝试或无效 JSON 必须失败关闭到 Primary，禁止为了提速用关键词快捷回复绕过真实工具链。
...
- Feishu 表情包的可信来源/置信度门禁、具体语义判断、中文口语 n-gram、元描述停用词、`avoidWhen`、同群/使用次数/最近使用排序、精确 file key 与别名解析，以及 80 条状态库 retention 去重统一由 `channels/feishu/stickers/sticker-selection-policy.ts` 维护。候选 evidence 的 `preferredFileKey` 会成为模型未返回候选分析块时的发送兜底，因此必须复用同一语义策略并传入当前请求文本；禁止用当前群最近出现或最高置信候选替代语义匹配。当前时间和媒体缓存存在性必须由 adapter 显式注入；策略模块不得读取文件系统、状态库路径或调用平台发送。
...
- Feishu CardKit v2/v1 能力探测、完整接口门禁和 create/stream/settings/final update 请求差异统一由 `channels/feishu/cards/cardkit-compat.ts` 维护；active card、创建中 promise、初始状态与双计时器释放由 `streaming-card-registry.ts` 管理；节流尾缘、打字机推进、tool 状态重启、sequence、创建中等待、完成状态/耗时和成功失败后的必清理由 `streaming-card-lifecycle.ts` 管理。adapter 只注入当前 REST Client、真实 CardKit 调用、最终 sticker/reaction 解析和出站引用持久化；禁止在 adapter 重新维护 throttle/typewriter/finalize 状态机、平行 Map 或独立清理分支。
