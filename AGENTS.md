...
- Feishu 结构化 mention 的 ID 字段必须统一兼容 `userId/user_id/openId/open_id`（保留 `unionId/union_id` 兼容），但字段归一化不代表可信：普通回复和 streaming card 收尾必须共用同一投递前门禁。结构化模型 ID 仍须与本轮原生 mention evidence 精确一致；显示名 resolver 生成的 ID 必须来自当前群官方成员/机器人唯一候选；bot-to-bot 返回 ID 还必须由 sender app_id 唯一关联。被拒绝后要移除对应裸 `@` 并保留文字语义，若用户本轮明确要求艾特或机器人回艾特失败，则必须明确说明未投递。广播 `atAll/at_all` 在存在独立 Owner 广播动作和确认协议前一律拒绝。
