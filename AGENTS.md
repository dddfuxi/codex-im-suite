...
- Feishu 出站 mention 的成员 payload 归一化、可 mention 用户 ID 过滤、机器人 sender `app_id` 与当前群 mentionable `member_id` 关联、候选按 ID 合并、证据优先级、同级歧义保留、history `<at>` 提取、DM receive ID 类型和原生标签构造统一由 `channels/feishu/mentions/outbound-mention-resolution.ts` 维护；入站原生 mention ID 匹配、text/post/card 结构化 `<at>` 递归检测、mention marker 清理、bot alias 归一化和 wake 裁决统一由 `inbound-mention-wake.ts` 维护。adapter 只负责读取当前设置、解析各消息类型的可见正文、从本轮原生 mention/当前 sender/受控历史索引/当前群成员 API 收集真实候选并执行平台收发。普通显示名 resolver 只接受当前回合明确执行命令；bot-to-bot 只允许在另一 bot/app 原生唤醒当前机器人后，按 sender app_id 唯一回艾特同一发送方。禁止把 app_id/bot_id 本身、模型生成 ID、无关显示名、reply metadata 或普通文本别名直接提升为可信 mention。
...
- Feishu @ 投递、事件订阅、回调、入站、通知送达等诊断文本，以及引用他人消息或规则说明里出现的 `@名字`，只能作为 evidence prompt 交给 agent 判断；不得触发出站原生 mention 补全或 resolver 检索。真实当前命令式请求（例如“请艾特某人让他看一下”）必须先进入 agent；Provider 前只可查询当前回合明确目标并注入不含平台 ID 的唯一性 evidence，Delivery 再按当前群真实候选复核。bot-to-bot 回合的唯一额外窄口是回艾特本轮原生唤醒当前机器人的发送方，且 sender app_id 必须唯一对应 mentionable member_id。
...
- Feishu 出站裸 `@显示名` 只允许两类受控补全：当前回合明确要求执行的具体显示名，或 bot-to-bot 回合中与已验证发送方机器人同名的返回 mention。两者都必须进入当前群官方候选 resolver 并唯一命中；模型单方面写出的其他名字、历史单独出现的名字、广播、关系描述和流程叙述不得触发查询或通知。AI 未输出裸 @ 时，普通明确命令可由 Delivery 为用户明确目标补 resolver 输入；bot-to-bot 不得替模型猜发送方名字。
