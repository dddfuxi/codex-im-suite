...
- Feishu 云文档的生成/列表意图、通用标题判定、正文改写 prompt、云链接与 OAuth callback 识别、Provider 前链接脱敏、阻塞文案和授权卡 reuse 去重统一由 `channels/feishu/documents/document-request-policy.ts` 维护。云链接必须通过 URL 解析确认 `feishu.cn` / `larksuite.com` 合法主机后缀和受支持资源路径，禁止用包含域名片段的宽松正则触发读取或 OAuth；manager 只保留 Host 调用、授权状态、审计、文档创建和交付编排。
