...
- Feishu CardKit v2/v1 能力探测、完整接口门禁和 create/stream/settings/final update 请求差异统一由 `channels/feishu/cards/cardkit-compat.ts` 维护；adapter 只传入当前 REST Client 并编排真实 IM 消息发送、流式状态、节流、最终展示与出站引用持久化。禁止为某个 SDK 版本在 adapter 内重新增加分支或复制 payload。
