...
- 本地语音或其他可选本机媒体运行时必须保持分层与显式启用：共享 Contract 只声明稳定状态/动作（语音四态固定为 `ready / optional_missing / blocked / error`），Core 只负责当前消息可信媒体证据、会话呈现策略和唯一用户可见终态，Runtime 独占依赖解析、Sidecar、媒体校验转换、ASR/TTS 与音色注册表，Control Panel 只渲染 Contract 并调用 Runtime；功能和输入/输出默认关闭，可选模型、Python、FFmpeg 或二进制不得随 npm/live/release 包安装，也不得在首条消息中隐式下载，受管依赖、授权音色和临时产物只能分别进入 `CTI_HOME/runtime-deps/speech`、`CTI_HOME/runtime/speech/voices`、`CTI_HOME/runtime/speech`，不得从 ST4 等外部项目缓存迁移或回退；`/voice off` 必须在再次 `/voice on` 前绝对阻止明确语音、入站语音和模型语音提示；原生媒体投递失败只能产生一次完整文字回退。合成产物只能由 Runtime 在本实例登记、受管根、普通文件和当前 Hash 全部复验后释放，Core 不得直接删除 Runtime 路径；平台上传前必须对实际上传字节复核 receipt Hash，清理失败只进入观察链，不能覆盖真实交付。完成前必须区分开发测试、live 同步/新 PID、目标硬件性能和真实渠道新消息验收。
