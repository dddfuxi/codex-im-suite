...
- 结构化判断协议以 `packages/contracts/src/decision.ts` 与 `schemas/decision.schema.json` 为唯一共享来源；Core 通过可选、只读的 `DecisionProviderHost` 提交已经裁决的问题。纯 Jev 动态模式可先调用独立的 `DecisionQuestionPlannerHost`，planner 只能在 classifier/严格 JSON、无工具、无工作区、无凭据和无发送权限的边界内生成一条受限 `noul / choice / score` 题目；Core 必须重新校验题型、长度、候选数量和敏感字段后，才交给 Runtime 的 Jev 适配器。Runtime 的 Jev 适配器本身只能调用专用 Decisions API，planner 与 Jev 任一关闭、缺密钥、超时、取消或非法结果都失败关闭，不回退固定分类或伪造概率。`off / shadow / assist` 与 `off / explicit / auto` 的配置枚举必须保持一致，普通请求保留 Primary。Feishu Decision Card 只显示实际题目的 `noul / choice / score` 和概率分布，不得生成 `cti-final.choices` 回调；新增替代 Provider、字段或展示入口时必须同步 Schema、定向测试、README、架构文档、开发日志和实施计划。
