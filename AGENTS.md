...
- 用户确实需要在 2–8 个有限、具体选项中做选择时，统一使用 `application/choice-prompts.ts` 清洗 `cti-final.choices`、签发短期回调并绑定原 channel/chat/user/session，使用 `channels/feishu/cards/choice-card.ts` 生成通用 Card 2.0 按钮；工作目录选择也必须复用该格式。模型只能提供可见 `label/description`，不得提供 `callback_data`、平台身份、命令或动作参数。自由输入、权限批准、Owner/高风险确认、密钥与身份解析不得被通用选择卡替代；不得靠普通正文里的“还是/或者/选一个”等词机械生成按钮。
