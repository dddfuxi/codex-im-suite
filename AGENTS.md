...
- Windows PowerShell 5.1 的用户 Profile 由 `scripts/windows-powershell-utf8-profile.ps1` 以受控区块维护，并为普通 `Get-Content` 设置 UTF-8 默认编码；任何 `powershell.exe -NoProfile` 调用仍必须在命令内显式设置 `$OutputEncoding`、`[Console]::InputEncoding` 和 `[Console]::OutputEncoding`，读取中文文件时显式传 `-Encoding UTF8`，不得假设用户 Profile 会生效。
...
- 多 Agent 协作协议以 `packages/contracts/src/agent-collaboration.ts`、`schemas/agent-collaboration.schema.json` 和 `config/agents.d` 为唯一声明来源；`packages/bridge-runtime/src/agent-workers` 独占 Worker Supervisor、NDJSON、受限 Provider、状态快照和协作 Host。专业 Agent 首期必须 `sideEffectLevel=none`，不得接收凭据、可信角色、平台目标或可执行路径，也不得返回工具/发送/写入动作；Primary Agent 与 Bridge 的唯一执行/投递边界不可下放。控制面板只能渲染 Manifest 和 Runtime 返回的节点/边，不得复制职责表或重新推断路由；新增 Agent、能力、节点状态或边类型时必须同步 Contract、Schema、C# DTO、Runtime 校验、Web view-model、架构文档和全链测试。新增或调整 Agent Manifest 目录时，还必须同步 live/portable/installer 复制入口、发布指纹目录发现和 `test-release-manifest-summary.ps1`，禁止开发版可见但 live Registry 缺失。
- 结构化判断协议以 `packages/contracts/src/decision.ts` 与 `schemas/decision.schema.json` 为唯一共享来源；Core 通过可选、只读的 `DecisionProviderHost` 提交已经裁决的问题。纯 Jev 动态模式可先调用独立的 `DecisionQuestionPlannerHost`，planner 只能在 classifier/严格 JSON、无工具、无工作区、无凭据和无发送权限的边界内工作：纯模式的 `answer_options` 目的必须生成 3–5 个完整答案/解决方案候选并固定为 `choice`，`/jev debug` 等意图判断仍可生成受限 `noul / choice / score` 题目；Core 必须重新校验题型、长度、候选数量和敏感字段后，才交给 Runtime 的 Jev 适配器。Runtime 的 Jev 适配器本身只能调用专用 Decisions API，planner 与 Jev 任一关闭、缺密钥、超时、取消或非法结果都失败关闭，不回退固定分类或伪造概率。`off / shadow / assist` 与 `off / explicit / auto` 的配置枚举必须保持一致，普通请求保留 Primary。纯模式卡片主内容显示答案/解决方案候选及概率分布，自动识别意图只作为辅助信息；Feishu Decision Card 不得生成 `cti-final.choices` 回调；新增替代 Provider、字段或展示入口时必须同步 Schema、定向测试、README、架构文档、开发日志和实施计划。
- Performance Agent 只能分析冻结时刻之前已经结束的协作回合；实时 `currentRun` 不得进入指标分母。每份建议必须记录真实 `evidenceRefs`、快照时间和 `analyzedThroughRunId` 水位，批次阈值按上次水位之后新增的已完成回合计算，Bridge 重启后从已持久化建议恢复节流时间，禁止达到总批次数后每轮重复生成同一窗口建议。
- Workflow 详细状态继续只保留有界最近窗口；跨窗口失败连续性必须使用 `workflow-failure-ledger/v1` 的单调 `sequence / retainedFromSequence` 和由稳定字段计算的 SHA-256 指纹。账本禁止保存正文、错误原文、动态 run/session/chat/user ID、绝对路径或凭据，写入失败只能影响观察链，不能阻断 Primary、Delivery 或真实 Workflow 终态。所有创建了协作 run 的 Provider 主路径都必须把真实 `workflowRunId` 回填到协作快照，关联失败时失败关闭为系统级趋势，不得猜测关联。
- 轻量会话协调器与多 Agent 协作 Coordinator 必须保持分层：前者属于 Provider 路由层，只在确定性硬门禁后的短消息上输出严格 `reply / delegate / clarify`，不得携带工作区、附件、工具或权限，也不得直接发送；后者只负责复杂任务的 Specialist 规划。manual official/external 下轻量协调器必须复用同一已选模型来源的常驻 Codex app-server、后台预热并按会话隔离 ephemeral thread，禁止退回每轮新建 `codex exec` 或混入本地辅助模型；auto failover 和 `local_api` 继续服从原选择链。确定性门禁必须依赖真实对象、附件、路径、平台动作或高风险意图，不能仅凭“检查 / 测试 / 看看”等弱动词把对机器人响应速度、在线接话或其他对话元信息误判为任务；真实 API、服务、文件、TAPD、Unity/MCP 等对象仍必须进入 Primary。轻聊 actor context 必须在后续 Markdown/Agent Home 标题处硬截断，只保留真实 sender/chat/wake/mention evidence，禁止把 Agent Home、工作区或工具规则带入受限 Prompt。明确任务直接进入 Primary，轻量协调器超时、低置信、工具尝试或无效 JSON 必须失败关闭到 Primary，禁止为了提速用关键词快捷回复绕过真实工具链。
- 通用轻聊分流器属于上述 Provider 路由层的可替换前置观察器，配置固定为 `CTI_LIGHT_CHAT_ROUTER_PROVIDER=coordinator|jev` 与 `CTI_LIGHT_CHAT_ROUTER_MODE=off|shadow|assist`。它只能在确定性轻聊门禁通过后运行，内部候选固定为 `light_chat / task / ambiguous`；Jev 仅调用 Decisions API `choice`，不能生成正文、按钮或执行动作。Assist 只有最高概率 `>=0.80` 且领先第二名 `>=0.15` 才能参与路由，高置信 `task` 可跳过 Coordinator 进入 Primary，失败、低置信和非法结果一律回退 Coordinator。新增 Provider 或阈值必须同步 contracts、Schema、定向测试、README、架构、开发日志和实施计划。
- 模型用量统一写入 `packages/contracts/src/usage.ts` 声明的 `cti-model-usage/v1`，由 Runtime `UsageMeter` 使用原子写入保存到 `CTI_HOME/runtime/model-usage.json`。记录只允许 Token、缓存 Token、延迟、Provider/模型、状态、路由、回退、费率和费用等脱敏字段；禁止保存完整 Prompt、回复、密钥或附件。Provider 回报费用优先，本地价格表兜底，价格未知必须保持 `null`/“未知”，用量账本写入失败不得阻断 Primary 或 Delivery。
- 本地轻聊 Provider 的健康状态必须在 Bridge 启动时通过低成本只读 endpoint 预热，不能把端点离线探测推迟到重启后的第一条用户消息；持久健康状态只有在 provider kind/base URL/model 身份匹配时才能恢复熔断，探针失败不得影响 Primary、工具、权限或外部执行链。
- 飞书执行卡片的 Agent 状态只能消费共享 `AgentCardProgressSnapshot` 脱敏 DTO；Runtime/Registry 必须保留本轮真实启动的 Coordinator、Specialist 与 Primary Agent 状态、稳定错误码和耗时，卡片晚于协作启动时也必须恢复最新快照。Shadow 和未实际注入 Primary 的 Assist 默认不在飞书卡片展示，只保留控制面板观测；只有已验证 section 确实交给 Primary Agent 的 Assist 才能在最终卡片默认折叠的“执行轨迹”中显示“已参与回答”，且不得重复展示普通 Primary Agent。禁止把完整 Workflow、findings、Prompt、evidence 原文、模型参数、路径或凭据塞进卡片。控制面板模式切换只能接受固定 `off / shadow / assist`，总览快捷开启固定进入 Shadow，写入 UTF-8 `config.env` 后必须真实重启 Bridge，前端不得先行伪造已生效状态。
- 多 Agent 协作快照属于观察链，不得因状态文件锁、临时文件或持久化异常阻断 Primary Agent、工具执行和 Delivery。只要本轮创建了协作 run，所有正常完成、提前返回、异常和取消路径都必须通过统一终态入口并由 `finally` 兜底收口；Bridge 启动时发现上个进程遗留的 `running / pending` 快照，只能标记为明确的重启恢复 `fallback / skipped`、清除 `currentRun` 且禁止自动重放，不得用恢复时刻减开始时刻伪造长耗时。Windows 运行态 JSON 写入必须复用带文件锁重试、受控覆盖兜底和过期普通临时文件清理的统一原子文本写入模块。Runtime、C# 控制面板与 WebView2 之间的 JSON 字符串必须保持合法 UTF-16：限长预览不得在代理对中间截断，状态持久化边界应递归归一化孤立代理项；控制面板读取历史坏记录时只在内存中替换非法转义并保留其余记录，投递异常必须在 JSON/WebView 边界吸收。只有检测到确凿非法代理项时，Runtime 才允许先备份再原子自愈状态文件；正常状态不得重复改写，禁止因单字段损坏删除或无备份重建整份状态库。
...
- Bridge Manager 的普通模型/工具回合必须经过跨会话全局并发闸门，默认最多 2 个执行槽，配置上限为 8；同一会话继续保持 FIFO。官方/外部 Codex 与本地 CLI 都必须有有限整轮超时，禁止用 `0` 恢复无限等待；断流恢复读取 rollout 必须节流、按 offset 增量读取并限制单次/待解析缓冲，避免同步整文件 I/O 阻塞事件循环。新增 Provider 或绕过 manager 的执行入口时，必须复用这些资源保护边界并补回归测试。
- 本地语音、歌声或其他可选本机媒体运行时必须保持分层与显式启用：共享 Contract 只声明稳定状态/动作（语音四态固定为 `ready / optional_missing / blocked / error`），Core 只负责当前消息可信媒体证据、会话呈现策略和唯一用户可见终态，Runtime 独占依赖解析、Sidecar、媒体校验转换、ASR/TTS、独立 Singing Host 与音色注册表，Control Panel 只渲染 Contract 并调用 Runtime；功能和输入/输出默认关闭，可选模型、Python、FFmpeg 或二进制不得随 npm/live/release 包安装，也不得在首条消息中隐式下载，受管依赖、授权音色和临时产物只能分别进入 `CTI_HOME/runtime-deps/speech`、`CTI_HOME/runtime/speech/voices`、`CTI_HOME/runtime/speech`，不得从 ST4 等外部项目缓存迁移或回退；`/voice off` 必须在再次 `/voice on` 前绝对阻止明确语音、入站语音和模型语音提示；唱歌只能使用受 benchmark 门禁的独立本地歌声模型，禁止用普通 TTS 冒充；控制面板必须分开选择/试听说话与歌声音色，试听媒体只能以无路径的受限回执通过 Runtime、C# 与浏览器校验；原生媒体投递失败只能产生一次完整文字回退。配置保存不等于 live 已加载；合成产物只能由 Runtime 在本实例登记、受管根、普通文件和当前 Hash 全部复验后释放，Core 不得直接删除 Runtime 路径；平台上传前必须对实际上传字节复核 receipt Hash，清理失败只进入观察链，不能覆盖真实交付。完成前必须区分开发测试、live 同步/新 PID、目标硬件性能和真实渠道新消息验收。
- 导入的授权参考音色必须跨重启持久化并出现在面板音色库；重命名只能修改 `reference` 的显示名，稳定 Profile ID、受管音频、授权证据和关联 benchmark 不得变化，并拒绝空名、超长名和重名；删除只能由 Runtime 根据 Profile ID 重新解析，只能删除 `reference`，并同步清理受管参考音频与关联 benchmark。预设音色不能重命名或删除，当前说话或歌声音色被删前必须先安全回退或清空配置，控制面板必须显示不可撤销的明确确认。
- 参考音色自动授权只能是默认关闭的通用 Runtime 策略，且仅适用于唯一 Bridge Owner 本人上传的可信原生回复录音；开启后不得重复询问录音权利、本机 TTS 用途和干净单人录音确认，但不得关闭逐字参考文本、重新 ASR、来源绑定、文件 Hash、音频质量或说话人相似度门禁。明确克隆请求必须进入 Bridge-owned SpeechHost，禁止 Primary 用 Bash、路径、普通附件或模型声明伪造导入；缺逐字文本时只报告该缺项，不能把 ASR 猜测当作用户确认。
- Feishu 表情包媒体文件的稳定命名、真实格式嗅探、兼容扩展查找、大小门禁和首份缓存复用统一由 `channels/feishu/media/sticker-media-cache.ts` 维护；adapter 只负责平台下载、失败冷却、sticker 状态和入站编排。sticker 下载必须始终绑定同一真实 `messageId/fileKey`，先按图片 transport 获取，失败后才允许回退到文件 transport，并以真实文件头重新确认 PNG/JPEG/GIF/WebP，禁止按 key 前缀、固定群聊或近邻图片猜测资源。媒体缓存模块不得读取语义 revision、选择发送候选或把记忆 media 目录提升为工作区。
- Feishu 表情包状态库的 record/store/tombstone/history-backfill schema、语义文本清洗、乱码剔除、置信度归一、视觉媒体 key 绑定校验、旧用户解释降级和 archive 时间约束统一由 `channels/feishu/stickers/sticker-store-schema.ts` 维护。adapter 只负责受控路径读取、备份恢复、原子写入和运行时编排；禁止在 adapter 或控制面板各自复制一套旧格式迁移与可信语义清洗规则。
- Feishu 表情包的可信来源/置信度门禁、具体语义判断、中文口语 n-gram、元描述停用词、`avoidWhen`、同群/使用次数/最近使用排序、精确 file key 与别名解析、自主发送会话冷却，以及 80 条状态库 retention 去重统一由 `channels/feishu/stickers/sticker-selection-policy.ts` 维护。候选 evidence 的 `preferredFileKey` 会成为模型未返回候选分析块时的发送兜底，因此必须复用同一语义策略并传入当前请求文本；禁止用当前群最近出现或最高置信候选替代语义匹配。明确“发个表情包”可绕过自主冷却；非明确请求必须经过确定性冷却，不能靠模型随机决定频率。当前时间和媒体缓存存在性必须由 adapter 显式注入；策略模块不得读取文件系统、状态库路径或调用平台发送。
- Feishu 表情包用户解释的文本解析、reply/同群近期目标绑定，以及 `user / vision / manual` 语义演进统一由 `channels/feishu/stickers/sticker-semantic-evolution-policy.ts` 维护。纯策略必须返回新的 store，不得原地修改输入；用户说法只能进入 `userAnnotation`，vision 必须与本轮真实媒体 file key 一致，manual 优先级最高，后续 vision 只能补空字段而不能降级人工语义。
- Feishu 表情包候选记录排序、受控 evidence DTO、候选行和视觉检查 prompt 统一由 `channels/feishu/stickers/sticker-candidate-evidence.ts` 维护。`userAnnotation` 不得进入候选事实 DTO；adapter 只负责最多按上限复用缓存图片、下载一个已验证候选并注入真实 attached file keys，禁止在 adapter 复制候选 prompt 或把未附加 file key 提升为视觉证据。
- Feishu 云文档的生成/列表意图、通用标题判定、正文改写 prompt、云链接与 OAuth callback 识别、Provider 前链接脱敏、Host 结果裁决、阻塞文案、授权卡 reuse 去重和 OAuth 审计输入统一由 `channels/feishu/documents/document-request-policy.ts` 维护。云链接必须通过 URL 解析确认 `feishu.cn` / `larksuite.com` 合法主机后缀和受支持资源路径，禁止用包含域名片段的宽松正则触发读取或 OAuth；同一回合只允许解析一次云文档 Host 结果，`resolved` 必须携带非空可靠正文 Prompt，缺失时失败关闭。审计不得包含授权 URL、卡片内容、device code 或 token；manager 只保留真实 Host 调用、Store 写入、授权恢复、文档创建和交付编排。
- Feishu 文档创建参数、记忆记录输入、导览 create/replace 计划与元数据、成功/失败交付文案统一由 `channels/feishu/documents/document-delivery-policy.ts` 维护。文档请求进入创建链后必须由文档结果收口：创建能力缺失或创建异常时只发送一条明确失败回复，禁止再回落发送原模型正文；错误外发前必须脱敏凭据和本地绝对路径。manager 继续负责真实 adapter 调用、文件/Store 写入、原子 meta 更新和 deliver，不得把这些副作用下沉到纯策略模块。
- Feishu 云历史的增量/全量分页、水位停止、成员名映射、删除/system/空消息过滤、sticker 采集时序和本地索引 upsert 统一由 `channels/feishu/history/indexed-history-sync.ts` 编排；当前 chat、查询、数量、时间与说话人范围到受控索引 Host 的调用统一由 `indexed-history-retrieval.ts` 收口，Host 不可用或 chat 身份为空时失败关闭，禁止恢复 direct recent-message 旁路；索引检索结果到 Provider prompt 的空结果、文档、定向引用和普通总结规则统一由 `indexed-history-prompt.ts` 构造；light context 的当前消息时间截断、reply 优先、self/system/删除过滤、短回复扩窗和“可能关联上文”锚点统一由 `light-context-selection.ts` 裁决；被回复历史消息的 image/sticker/file/audio/video/media/post/interactive 资源类型、精确 messageId/fileKey 绑定、顺序和去重由 `attachment-recovery.ts` 生成纯下载计划。adapter 只注入平台分页、成员查询、正文解析、sticker 采集、身份判断与索引能力，并负责真实 SDK/HTTP 下载、大小门禁、失败审计和附件注入；history 模块不得自行持有飞书凭据、执行网络下载、调用 Provider 或把本地历史索引提升为长期记忆事实源。
- Feishu 群名解析和普通增量历史维护不得阻塞无引用、无指代、无历史读取、无表情包请求的上下文无关轻聊；这类消息应先进入轻量协调器，群名与索引在 adapter 仍运行时延后后台刷新。原生 reply、短指代、续办、显式历史查询、头像/附件恢复和表情包候选等确实依赖平台 evidence 的回合仍必须在 Provider 前等待对应受控准备，不能为了提速丢失功能或把后台未完成状态伪装成证据。
- Feishu CardKit v2/v1 能力探测、完整接口门禁和 create/stream/settings/final update 请求差异统一由 `channels/feishu/cards/cardkit-compat.ts` 维护；active card、创建中 promise、初始状态与双计时器释放由 `streaming-card-registry.ts` 管理；节流尾缘、打字机推进、tool 状态重启、sequence、创建中等待、完成状态/耗时、原生媒体替代最终卡片和成功失败后的必清理由 `streaming-card-lifecycle.ts` 管理。只有原生 sticker 已真实发送成功、纯策略确认它可完整替代冗余文字且回合为 completed 时，才允许撤回机器人自己的临时进度卡；撤回失败必须回退为正常最终卡片，不能留下永久“处理中”状态。用户可见反馈默认延迟必须由 channel 通过通用 adapter 能力提示声明，并允许显式 Store/环境配置覆盖；`0ms` 必须表示当前调用栈同步启动，禁止仍用 `setTimeout(0)` 排到 manifest/Provider 同步初始化之后。Feishu 两段式 CardKit 首卡应立即启动以隐藏平台 RTT，确定性即时回复仍须在建卡前直接收口，不能为提速机械闪卡。adapter 只注入当前 REST Client、真实 CardKit/IM 调用、最终 sticker/reaction 解析和出站引用持久化；禁止在 adapter 重新维护 throttle/typewriter/finalize 状态机、平行 Map 或独立清理分支。
- Feishu 卡片横幅头图只能由可选 `cti-final.card_hero` 选择同一 envelope 的 `images` 中一张已获准交付的图片；模型不得提供可信 `image_key`、URL、平台身份或额外路径。Bridge 必须在输入 evidence、执行证据、编码和文件存在性门禁后上传，并验证真实文件头、尺寸、大小与非符号链接；普通卡、流式终态卡和通用选择卡复用同一 Card 2.0 图片元素。Card JSON 2.0 通栏必须使用当前支持的图片属性，禁止恢复 `size=stretch_without_padding` 等旧枚举；组合卡被平台拒绝时先去头图重发原正文与按钮，仍失败才用飞书富文本回退。只有平台发送或更新成功回执才能去掉同图的独立附件，上传或嵌入失败必须保留普通图片回退，非飞书渠道不得丢图。
- Feishu 通用看盘式分析呈现只能通过可选 `cti-final.analysis_view` 提交标题、结论、受控语气、最多 6 项指标和最多 4 个可见文本分区；清洗协议统一由 `application/analysis-view.ts` 维护，飞书映射统一由 `markdown/feishu.ts` 维护。协议必须先过滤无效/重复指标再计入展示上限，同名分区合并并对条目去重；飞书指标使用移动端友好的“指标 / 状态与信号”双列表格，当前值与变化沿用同一 tone。结构化标题或结论与普通正文完全重复时只折叠代码围栏外的重复展示行，代码块与独有依据不得丢失。该结构必须适用于市场、项目、服务、故障、投票和测试等通用多指标态势，禁止按股票词、固定机器人或固定群写死；轻聊和单一事实不得机械套模板，也不得为填充版式伪造价格、百分比、状态或趋势。模型不得提供 Card JSON、颜色、URL、命令、路径、回调或平台身份；普通正文必须保留为补充说明和非飞书回退。后置 evidence、权限或交付门禁将回复改为未完成时必须清除旧分析视图，头图、分析视图和有限选择按钮继续复用同一 Card 2.0 生命周期。
- Feishu 任务卡片的阶段轨迹、工具相对时间和证据状态必须来自 `streaming-card-lifecycle.ts` 记录的真实时间与 `ToolCallInfo.status`；无工具回合只能标记为文本回复，缺少平台回执、live hash/PID 或现场复测 evidence 时不得显示“现场已验证”。最终正文保持结果优先，工具轨迹默认折叠，禁止为了炫酷泄露原始命令、绝对路径、内部协议名或伪造执行进度。
- 用户确实需要在 2–8 个有限、具体选项中做选择时，统一使用 `application/choice-prompts.ts` 清洗 `cti-final.choices`、签发短期回调并绑定原 channel/chat/session，使用 `channels/feishu/cards/choice-card.ts` 生成通用 Card 2.0 按钮；普通选择默认继续绑定原 user，工作目录选择也必须复用同一格式。只有模型在用户明确要求群体参与时声明受控 `choice_session`，才可进入 `vote / claim / parallel`：参与范围绑定当前 chat，点击者来自原生 callback 并优先经当前群成员 API 复核；成员接口暂时失败时只允许对低风险群体选择使用原生 operator + chat evidence 降级，权限、Owner、高风险、密钥和身份流程不得降级。投票按 Bridge 签发的绝对截止时间收集并到期统一回调，抢选必须原子单赢家，多人分线按匿名参与分支分别回调；parallel 初始入口允许当前群可信成员各进入一次，但某参与者进入分线后的连续卡必须降为 `single_user` 并绑定该真实点击者，同时只向 Provider 延续匿名 branch key，禁止其他成员误入或把平台 ID 当分支键。选择、计票、原卡 message ID、截止时间、匿名续办分支、待投递终态和 consumed tombstone 必须原子持久化，重启后幂等恢复，禁止每秒高频刷卡或产生重复终态。模型只能提供可见 `label/description` 与受控模式/时长，不得提供 `callback_data`、平台身份、命令、URL 或动作参数。连续有限选择继续用 `choice_flow continuous active/complete` 显式管理；活动流程漏掉 choices/complete 时只允许一次禁工具、无副作用的 response-only 协议修复，禁止从任意 Markdown 的 A/B/C 或编号猜造按钮。自由输入、权限批准、Owner/高风险确认、密钥与身份解析不得被通用选择卡替代；不得靠普通正文里的“还是/或者/选一个”等词机械生成按钮。
- 全员投票应在首次合法点击时冻结官方群真人成员快照；全部稳定成员身份均已选择时立即原子收口，否则按 Bridge 签发的绝对截止时间兜底。成员接口失败时禁止按当前票数、群名或模型文本猜测总人数；参与者快照、收口原因和待投递终态必须与选票一同原子持久化，重启后继续使用同一覆盖口径。
- 通用低风险动作的“风险 × evidence × 平台验证 × 安全档位”纯裁决统一由 `adaptive-action-policy.ts` 维护；`strict / balanced / fluent` 只能影响低风险降级，不能取消身份冲突、真实歧义、Owner、授权、密钥或高风险确认。Feishu 出站 mention 的成员 payload 归一化、可 mention 用户 ID 过滤、机器人 sender `app_id/open_id/user_id/union_id` 与当前群 mentionable `member_id` 唯一求交、候选按 ID 合并、证据优先级、同级歧义保留、history `<at>` 提取、DM receive ID 类型和原生标签构造统一由 `channels/feishu/mentions/outbound-mention-resolution.ts` 维护；同一人类消息原生点名多个参与者并指定“谁先开始、发言后 @ 对方”时，当前机器人、先手、当前轮次和唯一对方必须在 Provider 前由 `orchestrated-interaction.ts` 统一解释，先手名称是发言角色而不是出站目标，非先手机器人应等待先手原生 mention 后再接话；“他/她/对方/刚才那个人”等当前回合明确执行指代到真实人物 evidence 的纯裁决统一由 `contextual-mention-resolution.ts` 维护；入站原生 mention ID 匹配、text/post/card 结构化 `<at>` 递归检测、mention marker 清理、bot alias 归一化和 wake 裁决统一由 `inbound-mention-wake.ts` 维护。adapter 只负责读取当前设置、解析各消息类型的可见正文、从本轮原生 mention/当前 sender/受控历史索引/当前群成员 API 收集真实候选、按 evidence ID 查询当前群身份并执行平台收发。上下文指代只能选择本轮真实 evidence ID；优先按 ID 官方复核，`balanced / fluent` 下成员接口临时失败时可只对强平台 evidence 的同群单人 mention 做可审计降级。模型 ID 本身不可信，明确查无成员、身份冲突和多候选仍必须拦截或澄清。bot-to-bot 只允许在另一 bot/app 原生唤醒当前机器人后，按事件真实 sender 身份与当前群官方候选唯一求交并回艾特同一发送方；不同 ID 指向不同成员时必须失败关闭。禁止把 app_id/bot_id 本身、模型生成 ID、无关显示名、reply metadata 或普通文本别名直接提升为可信 mention。
- Feishu 入站 FIFO、等待消费者、按稳定 messageId 撤回删除和 open/close 清理统一由 `channels/feishu/lifecycle/inbound-queue.ts` 维护；adapter 继续负责真实 WS/SDK/P2P 调度。停止或启动失败时必须丢弃未处理旧消息并唤醒等待者，close 后迟到事件不得重新入队；“是否开放接收”与“当前消费者是否等待未来消息”必须分开裁决，避免未运行 adapter 在空队列永久等待。
- Feishu P2P 私聊补捞的立即运行、固定间隔、跨重启 single-flight、timer 释放、停止后旧状态抑制，以及 deleted/system/self/seen/旧水位过滤与时间排序统一由 `channels/feishu/lifecycle/p2p-polling.ts` 维护。adapter 只注入已注册 P2P chat、平台分页、当前 bot/seen 判断、消息转换和运行审计；平台 fetch 返回后必须再次检查 adapter 仍在运行，禁止停机中的旧轮询继续解析、下载或恢复消息。
...
- 模型自我识别属于 Agent Kernel 的 Runtime evidence：Provider 必须从本轮实际提交给 `thread/start` 或本地 CLI 的模型 ID 生成受控身份段；历史、近邻、记忆和模型自报不能覆盖当前证据。提交 ID 只证明本轮提交的路由值，不证明代理后的物理模型或厂商；缺少值时不得猜测，classifier 仍保持严格 JSON。
- Feishu 文本分区、引用、粗体/斜体/删除线、列表和下划线兼容降级属于 Delivery Layer：模型侧规则必须先登记为 `delivery_layer.feishu_text_presentation`，仅在飞书回合注入；最终 Markdown 统一经过 `markdown/feishu.ts` 规范化。飞书 Card 2.0 未明确支持的 `<u>/<ins>` 不得原样外发，应在代码块外降级为受支持的强调色加粗，代码示例保持原文；短聊天不得为了形式强制套标题或报告模板。
...
- 用户原生回复已完成结果并用短句修改预算、数量、样式、筛选或其他约束时，必须优先从同一 `channel/chat/messageId` 的耐久 outbound ref 恢复原始请求、终态和结果；可见卡片标题即使出现在摘要中也不能导致整段续办上下文被去重。只有受控耐久摘要真实恢复后，短约束修改才允许继承上一轮 `ExecutionRequirement`；产物任务必须继续要求本轮新产物和成功工具证据，模型只说“我会重做/按此重排”不得作为完成终态。缺证据时最多执行三次正式尝试（首次加两次递进恢复），第二次恢复必须切换兼容路线而不是重复口头承诺；未知工具已经启动或可能产生副作用时不得为凑次数整轮重放。普通感谢、评价和未恢复卡片标题不得触发继承，也不得按具体预算、榜单或固定关键词写特例。
...
- 产物类执行证据不能只按工具名称裁决，也不能只信模型声明路径；只有最终结果声明、当前 attempt 成功工具结果、真实新文件时间、允许工作区/回合目录和非符号链接边界共同验证后，才可替代特定工具 family 证据。普通只读工具返回的 `path` 不得自动登记为生成产物。Workflow 已进入 `executing / finalizing / retrying` 后发生 Bridge 重启时，禁止仅凭原 prompt 自动重放；保留输入供用户显式手动重试，并收口旧 `retrying`，避免副作用和恢复链级联。
- 工具 family 证据必须识别真实的嵌套执行：当受控 Bash/PowerShell 在工具输入中确实向本机 MCP 端点发出 JSON-RPC 请求，并在同一 `tool_use/tool_result` 中获得无内层失败的成功回执时，应按实际 MCP action/resource 归一化证据，不能只因外层工具名是 `Bash` 判为未执行。模型正文、普通回显、只出现 Unity/MCP 词、`success=false / ok=false` 或无法关联到同一工具对的输出均不能补证据；Primary Agent 负责判断可信事实是否覆盖用户需求，确定性层只验证事实来源与安全边界。
- 同一入站 `sourceMessageId + turnId` 只能产生一个用户可见终态。活跃回合的 Provider 可恢复失败必须通过严格 `retry_advice` 交给 Conversation Engine 在原回合、原卡片内续跑；不得先定稿失败再排后台自动重试或主动补发第二份结果。缺证据重试与 Provider 恢复各最多一次；已有写入、发送或未知副作用时不得整轮重放，只能恢复经 TurnStorage 完整验证的受管产物，否则明确等待人工重试。人工重试必须继承原执行要求和受管输入 evidence，缺失或过期时失败关闭。
- 控制面板终止当前回复必须通过 Runtime 受控命令通道复用 Bridge 内存中的真实 AbortController；必须以 Workflow run、sessionId、原始 turnId 和可用的 channel/chat identity 共同匹配，禁止通过停止/重启 Bridge、直接改状态文件或宽松查找实现。终止只影响目标回合，收口为唯一 `cancelled` 终态且禁用自动重试；迟到 Provider 结果不得覆盖取消状态或继续投递，已结束、缺少 turn identity、身份冲突和非活动 run 必须幂等返回或失败关闭。
- Workflow 失败归因必须由 Runtime 生成稳定、脱敏的 `source/category/code/summary`，区分 Provider 认证、额度、协议、参数、瞬时失败与工具依赖路径、模块加载、宿主 helper/runtime 等原因；自动化、控制面板和性能分析消费诊断码去重，不得依赖绝对路径、原始异常全文或把工具连锁报错覆盖成主因。
...
- Markdown 自维护不能取消 Owner/Operator 角色、密钥保护、平台授权、真实工具证据、高危动作确认等代码级硬约束。所有自维护写入必须先获取 `.cti-self-history/write.lock`；即使 mtime 超时，持锁 PID 仍存活时也不得删除。correction 自维护可在 Provider 前同步影响本轮；outcome 自维护必须在最终投递后旁路执行，不得继承本轮 task abort signal 或阻塞消息 FIFO、session lock 和下一轮处理。核心文档只允许稳定 key、`baseHash` 和受控 patch 修改 `Agent 自维护规则` 块，禁止 classifier 整篇 replace 用户主体。多目标提交必须先持久化 transaction manifest 与 before-image，失败时回滚，进程崩溃后在下次持锁时恢复未完成事务；索引重建属于派生数据，失败不得回滚已提交事实源。
...
- 聊天工作区切换的 `sessions.json` 与 `bindings.json` 必须作为跨进程事实源事务更新：锁内先回读磁盘、修改目标记录并原子写回，迟到的旧 session 回执不得覆盖新绑定；启用的结构化项目根与 legacy allowlist 必须在会话路由层合并为同一授权上界，但未选中项目不得因此默认挂载。切换成功前必须走下一条普通消息使用的同一路由策略复验，只有聊天 binding、实际 session 和两处 cwd 都与目标一致，才允许回复成功。重复或重叠 Bridge 进程仍应单独审计，文件事务只负责防止状态回退，不能伪装成单实例运行。
...
- 同一批次多个计划任务到期时，必须先完成整批稳定 slot 准入，再有界并发执行；固定 `notify/check_in` 不得被慢 `agent_turn` 串行阻塞。并发上限必须复用运行时配置，运行账本应记录计划时刻到实际排队的延迟，禁止按具体任务名称或固定时刻写优先级特例。
- `notify`、`check_in`、`agent_turn`、`controlled_tool` 必须走同一 Host、权限和审计边界。`check_in` 每次运行使用独立 `slotKey` 打卡账本，点击者必须绑定原生卡片回调、真实投递 message ID 和当前会话，群成员经平台 evidence 校验后按用户去重；不得用普通回复、表情、投票或旧 reminder 完成状态伪装打卡。`controlled_tool` 必须 Owner，且只能信 Runtime 工具注册表声明的幂等性；模型提供的 owner、role、chatId、openId、workspace、sourceSessionId 等字段一律不可信。
- `cti-scheduled-task` 必须兼容模型把“向当前群发送固定消息”表达为 `taskAction.kind=direct_message` 的合理协议变体，但只能归一为当前入站会话绑定的 `notify`；模型给出的 `targetId / targetType / chatId / userId` 只能审计后忽略，指定用户的私发不得静默改投当前群。字段校验失败必须指出真实无效层级或 kind，禁止把已有完整字段统一误报成“缺少 name、schedule 或 taskAction”。
- `cti-scheduled-task` 的 cron 必须兼容规范对象与显式 `CRON_TZ/TZ` 字符串；裸 cron 字符串不得猜测时区。模型已经给出语义唯一的时间变体时，应在纯解析层受控归一：`at.datetime` 只在携带显式 RFC3339 偏移或独立 timezone 时转为 `at.at`，`once + delay*` 按真实动作解析时刻转为单次 UTC，`interval + every*` 转为 `everyMs + anchorAt`；转换字段必须进入审计。无偏移本地时间、非正间隔、无 kind/type 或未知枚举仍失败关闭，并指出具体字段。`agent_turn` 缺省 `sessionMode` 时只能降为无工作区 `isolated`，未知值失败关闭，只有显式 `bound` 才进入 Host 的可信工作区重建。
...
- Windows 下所有 live Bridge 普通 `stop / restart / uninstall-service` 必须统一通过 `packages/bridge-runtime/scripts/daemon.ps1`，由 `workflow-drain.ps1` 先等待 `running/retrying` Workflow 排空；`dist/daemon.mjs` 是长驻 Runtime 入口，不是控制 CLI，禁止向它传 `start / stop / restart`，否则会误启动第二个 Bridge。超时必须延期终止并保留原进程，只有明确诊断为卡死后的人工恢复才可显式使用 `-Force` 或受控 force 配置。控制面板和机器人受控入口必须传固定来源，drain 审计只能记录动作、来源、活动数量、阶段和裁决，不得记录正文、身份或绝对路径。`config.env` 中的 drain/force 配置优先于父进程继承环境。启动/重启命令会脱离出长期存活的托管进程组；Supervisor 完成 PID、状态和进程检查后必须先写进程隔离的启动回执，daemon 只可在收到该回执后结束仍被后台句柄拖住的短命包装器，不能等待固定超时或递归终止已经启动的 Supervisor/Bridge。控制面板对 start/restart 必须使用文件型命令输出，禁止匿名 stdout/stderr 管道让 `WaitForExitAsync` 被长驻后代拖住；stop/status/logs 仍保留普通捕获和整树超时清理。控制面板不得只凭 start 命令退出码宣称重启成功，必须核对 Supervisor/Windows Service、Bridge 新 PID、匹配 run 的运行审计、近期心跳和已启用且可审计的回调通道；任一层未恢复应失败并明确指出。新增或修改 drain/重启脚本时必须同时用“包装器不退出但受管进程存活”的集成回归验证，再核对 live、portable、installer 复制链、发布指纹和控制面板复制安装漂移探针。
- 机器人受控 Bridge 重启允许用户用短句“重启”确认，但只限当前消息原生回复了唯一、可靠恢复正文的机器人/应用重启邀请，且邀请明确指向 Bridge 并要求回复重启；普通近邻、memory、推测 continuation、不可读卡片壳或用户消息都不能授权。没有这条强证据时，裸“重启”继续失败关闭；Owner、固定 `restart_live` 动作和真实调度回执门禁保持不变。
...
- 飞书权限申请必须按当前动作与明确请求字段最小化：群成员资料意图不能只依赖“查/看/列出”等命令式动词，“是什么/有哪些/分别属于/各自在什么部门”等内容问句，以及明确当前群成员和资料字段、长度受限的短名词式低风险读取，也必须进入同一字段计划；教程、权限元问题和不明确对象不得误触。群成员“身份 / 用户还是机器人”只消费官方群成员列表的用户/机器人分类，禁止为此调用 Contact、展示未请求的部门/职位/状态或生成字段 scope；只有用户明确要求职位、激活状态或部门时，FeishuAdapter 才用当前 bot token 查询对应字段，禁止转成普通用户 OAuth 或只给教程不执行。一个 API 返回的多个兼容 missing scope 只能作为 alternatives，统一策略只选一个风险最低的只读/base scope。Contact 用户接口成功不等于已请求字段已授权，必须只对本轮请求字段按属性存在性区分“未授权”和“未填写”，激活状态读取 `user.status.is_activated` 并兼容旧顶层字段；职位/状态缺失使用 `contact:user.employee:readonly`，部门 ID 使用 `contact:user.department:readonly`，部门名称使用 `contact:department.base:readonly`，未请求字段不得参与权限排序。部门接口权限错误不得静默吞成空值，单个成员或部门失败后继续处理其余明确目标。头像请求必须先解析用户/机器人/全体/当前发送者/当前群具名目标，“你们/各自/自己的头像”等机器人集合自指不得扩张到真人；“我/我的头像”必须绑定本轮入站 sender ID，并与当前群官方 roster 唯一求交，禁止退化为姓名截取；“现在你能看到头像了吗”等能力/状态问句不得把动词片段误提取为成员名。其他具名显示名只允许作为查找提示，当前群官方 roster 精确匹配失败后只允许唯一的规范化包含关系命中，找不到、同名或多个相关候选仍失败关闭，不得扩大查询、跨群信任模型 ID 或申请权限。成员 evidence 官方 API 仅对网络异常、408/425/429 和 5xx 自动续试一次，403、身份冲突、通讯录数据范围和确定性 4xx 不重试；头像尺寸候选应逐个尝试，单个尺寸失败不阻断其他官方候选。当前机器人头像优先复用 Bot v3 与启动缓存，不得误走 Application v6 后反推 `admin:app.info:readonly`。用户 OAuth 一次只允许一个有真实 API/schema 证据的精确 scope，Core 与 Runtime 必须拒绝 `lark-cli auth login --recommend`、任何 `--domain` 和多 scope。应用 scope、通讯录数据范围或版本发布需要管理员时，先向 Owner 说明唯一下一项并询问，未确认前不得打开申请入口、代申请或声称已通过审核。任何放宽和续试都必须基于通用语义、当前群真实 evidence 与可验证兼容路径，禁止写死姓名、原句、群或权限结果。
...
- Feishu 群聊 `require_mention` 下，主入口只接受原生 @ 当前 bot 的消息：优先按 `message.mentions` 中的 `open_id/user_id/union_id` 与当前 bot 身份精确匹配，飞书长连缺失 `mentions` 时仅允许正文结构化 `<at ...>` / `tag=at` 携带同一身份 ID 作为兼容证据。人类只原生 @ 当前 bot 且没有正文/附件时属于明确轻聊唤醒，应由 `inbound-mention-wake.ts` 归一化后交给当前已选 Provider 的受限轻量协调器自然接话，adapter 不得写死最终回复；同一动作带原生 reply/root 时仍优先处理被引用目标，其他 bot/app 的空 @ 不得借此绕过 bot-to-bot 内容和循环门禁。唯一窄例外是用户用飞书原生 reply/引用“当前 bot 已发送的消息”补发 `sticker` 或 `image`：必须通过本地 `outbound-refs` 或被回复消息 sender 证明确为当前 bot，才可作为本轮媒体交互唤醒并进入表情包/图片识别链。普通文本 reply、回复未知/其他人的消息、文本别名、显示名、其他 bot/app sender 或 @ 其他机器人都不能替代原生 @ 唤醒；这类内容只作为审计或上下文证据，不触发 agent。
...
- Feishu CLI / lark-cli 默认只能作为 `/feishu` 能力诊断或人工 API 调试证据；审批提交等确需用户身份的私有资源动作，仅可通过 `cti-feishu-cli-user-auth` 治理链路执行：按当前真实 sender 的 `userId` 使用独立运行态配置目录，禁止复用 Owner 或其他成员 token，缺少身份/隔离配置时失败关闭。即使配置了 `CTI_FEISHUCLI_ENABLED=true`，也不得把普通 lark-cli 当成入站、@ 判断、成员 resolver、云聊天记录同步或消息发送主链路。`service.feishuCli` 只是历史兼容 id 的 Bridge Skill 更新单元，不得因名称恢复 CLI 快捷 provider；飞书云历史必须由 FeishuAdapter 的 OpenAPI / 长连接 evidence 证明。
...
- Feishu 跨群、跨会话或按 `chat_id/session_id/targetId` 发送消息必须仍走 `cti-direct-message` 受控动作，但只有目标与本轮可信来源 `chatId` 不同时才属于 owner-only 跨会话操作；模型目标 ID 与来源 `chatId` 完全一致时应归为当前会话，不能仅因 `targetType=chat` 误升为跨会话。用户用“现在测试一次 / 继续发送 / 确认”等动作短句原生回复本机器人已持久化结果时，可从同一 outbound ref 的可靠续办摘要继承“进入目标与权限裁决”的授权，普通历史、引用文本和不可读卡片壳不得继承；当前会话可直接受控发送，真正跨会话仍必须由 adapter resolver 得到唯一目标，向发起 owner 展示目标名称、类型和平台 ID 并要求确认。确认回调必须来自同一源会话、同一 owner，确认后才调用 adapter 发送。确认卡和源会话结果不得回显待发送正文；目标不唯一、权限不足、确认过期或 resolver 不支持时必须返回未完成，不得让模型口头声称已跨会话发送。
- Feishu @ 投递、事件订阅、回调、入站、通知送达等诊断文本，以及引用他人消息或规则说明里出现的 `@名字`，只能作为 evidence prompt 交给 agent 判断；不得触发出站原生 mention 补全或 resolver 检索。真实当前命令式请求必须先进入 agent；具体姓名可走当前群官方唯一解析，“他/她/对方/刚才那个人”等指代只能由 `contextual-mention-resolution.ts` 绑定到本轮 `TurnEvidenceEnvelope` 中一个真实人物 evidence，再由 Delivery 用当前群官方成员/机器人复核同一身份。模型只能选择 evidence，不能直接授权模型输出的 ID。bot-to-bot 回合的额外窄口仍是回艾特本轮原生唤醒当前机器人的发送方；优先使用 sender app_id，也必须兼容事件只返回 open_id/user_id/union_id 的真实变体，所有可用 ID 与当前群 mentionable member_id 求交后必须唯一且无冲突。
...
- Feishu 出站裸 `@显示名` 允许三类受控补全：当前回合明确要求执行的具体显示名；指代请求中由本轮真实人物 evidence 唯一解析并经当前群官方复核的目标；bot-to-bot 回合中与已验证发送方机器人同名的返回 mention。模型单方面写出的其他名字或 ID、无 evidence 绑定的历史名字、广播、泛关系描述和流程叙述不得触发查询或通知。AI 未输出裸 @ 时，普通明确姓名命令或已唯一解析的上下文目标可由 Delivery 补 resolver 输入；多候选必须最小澄清，不能随机选人。
- Feishu mention 模型协议必须兼容字符串显示名、仅含 `name/user_name` 的对象和含 `userId/user_id/openId/open_id/id`（保留 `unionId/union_id`）的结构化对象。字符串和 name-only 对象只是不可信的“目标选择提示”，不能直接成为平台身份；通用 `id` 与其他结构化模型 ID 同样只是不可信候选，仍须与本轮原生 mention evidence 精确一致。普通回复和 streaming card 收尾必须共用同一投递前门禁；显示名 resolver 生成的 ID 必须来自当前群官方成员/机器人唯一候选；bot-to-bot 返回 ID 还必须由 sender app_id 或事件原生 open/user/union ID 与当前群候选唯一关联，身份冲突不得降级。被拒绝后要移除对应裸 `@` 并保留文字语义，若用户本轮明确要求艾特或机器人回艾特失败，则必须明确说明未投递。广播 `atAll/at_all` 在存在独立 Owner 广播动作和确认协议前一律拒绝。
...
- `config.env` 是 Bridge 启动配置唯一事实源；Windows live 重启必须在创建 Provider 前用文件值覆盖父进程继承的旧环境，禁止让控制面板或 supervisor 的旧 `CTI_*` 值覆盖刚保存的模型、推理强度、来源或端点。现场验收至少核对生成的 Codex Home 配置或真实 Codex 子进程参数。
- Owner 通过文字或可信语音修改控制面板设置时，必须复用共享设置 Contract、当前 Runtime 目录和 `cti-panel-settings` Host；不得把 ASR 文本、固定中文句子、模型生成 env 名或任意键值直接写入配置。敏感项只显示配置状态，写入必须保留未知字段与注释并做 Owner、类型、枚举、范围、路径和乐观版本校验；只有真实写入回执才能列出变更并安排回复后的固定重启。
- Bridge 的 Codex SDK/CLI Home 默认不得继承 Codex Desktop 全局插件；Computer Use、Browser 等桌面插件可能依赖 desktop native helper/pipe，CLI/SDK 环境不可用时会形成路径缺失和模块加载连锁失败。只有确认目标插件兼容当前 Bridge 运行时后才可显式设置 `CTI_CODEX_INHERIT_GLOBAL_PLUGINS=true`；生成 Home 清理 junction/symlink 时只解除生成入口，不得递归触碰用户全局插件缓存。Provider Prompt 也不得猜 skill 路径或手工执行插件缓存脚本。
...
- Unity、Blender、MCP 等外部编辑器里的场景、Prefab、节点、组件和对象写入默认属于 `tool_required`，不能因为用户附了参考图或动词包含“创建/保存”就升级为 `artifact_required`；只有用户明确要求图片、文档、表格等输出产物时才要求 Runtime 验证的新文件。普通 Shell 成功不能冒充外部编辑器证据，只有受控 CLI/MCP 的真实成功回执才能满足对应 family。
- 官方 Codex SDK 在最终协议前断流时，不得立即结束用户卡片并放任同一 Agent 后台继续。应优先按同一受管 thread 的 rollout 等待真实 `task_complete` 并续接工具/最终事件；不得重放已经可能产生副作用的原请求。恢复超时、rollout 不可用或用户取消时，必须先中止底层 run，再输出明确失败终态。
