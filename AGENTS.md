...
- `cti-scheduled-task` 的 cron 必须兼容规范对象与显式 `CRON_TZ/TZ` 字符串；裸 cron 字符串不得猜测时区。`agent_turn` 缺省 `sessionMode` 时只能降为无工作区 `isolated`，未知值失败关闭，只有显式 `bound` 才进入 Host 的可信工作区重建。
