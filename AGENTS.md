...
- 多 Agent 协作协议以 `packages/contracts/src/agent-collaboration.ts`、`schemas/agent-collaboration.schema.json` 和 `config/agents.d` 为唯一声明来源；`packages/bridge-runtime/src/agent-workers` 独占 Worker Supervisor、NDJSON、受限 Provider、状态快照和协作 Host。专业 Agent 首期必须 `sideEffectLevel=none`，不得接收凭据、可信角色、平台目标或可执行路径，也不得返回工具/发送/写入动作；Primary Agent 与 Bridge 的唯一执行/投递边界不可下放。控制面板只能渲染 Manifest 和 Runtime 返回的节点/边，不得复制职责表或重新推断路由；新增 Agent、能力、节点状态或边类型时必须同步 Contract、Schema、C# DTO、Runtime 校验、Web view-model、架构文档和全链测试。
