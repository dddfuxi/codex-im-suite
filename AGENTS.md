...
- Feishu 出站 mention 的成员 payload 归一化、可 mention 用户 ID 过滤、候选按 ID 合并、证据优先级、同级歧义保留、history `<at>` 提取、DM receive ID 类型和原生标签构造统一由 `channels/feishu/mentions/outbound-mention-resolution.ts` 维护；入站原生 mention ID 匹配、text/post/card 结构化 `<at>` 递归检测、mention marker 清理、bot alias 归一化和 wake 裁决统一由 `inbound-mention-wake.ts` 维护。adapter 只负责读取当前设置、解析各消息类型的可见正文、从本轮原生 mention/当前 sender/受控历史索引/当前群成员 API 收集真实候选并执行平台收发；只有当前回合明确要求执行、且可提取具体显示名的 mention 命令允许查询当前群官方成员/机器人，查询结论以不含平台 ID 的受控 evidence 进入 Provider，Delivery 必须再次按真实候选唯一核验。禁止把 app_id/bot_id、模型生成 ID、普通叙述中的裸 `@`、历史单独出现的名字、广播受众、关系称呼、reply metadata 或普通文本别名直接提升为可信 mention。
