...
## Local speech operational boundary (development 0.4.0)

Local speech is a Feishu-first development feature and is disabled by default. Do not infer live availability from source code, a successful build, or unit tests.

- With no session override, trusted inbound Feishu audio is transcribed locally and defaults to a voice reply. Ordinary text defaults to text. `/voice off` is a hard disable until `/voice on` is sent again. Never use natural-language keyword matching as the authorization or completion gate: only a bounded Primary speech intent may request voice presentation, and Bridge must still combine it with trusted media evidence, session state, role, risk, and Runtime readiness.
...
- Qwen3-TTS is an audio Runtime only. Default to the 1.7B CustomVoice model with Serena, expose the 0.6B CustomVoice model as an explicit low-VRAM choice, and use Base variants only for authorized voice cloning. Do not replace the Primary Agent, SenseVoice ASR, or the independent ACE-Step Singing Host.
- Optional models, FFmpeg, Python, and speech binaries are not npm/live/release dependencies and must never be downloaded by the first speech message. Model collections require pinned revisions and per-file hashes and must be installed transactionally after an explicit operator action. Do not claim the Qwen Runtime is ready until its isolated Python/CUDA environment and the selected model pass live health and model/revision/hardware benchmark gates.
