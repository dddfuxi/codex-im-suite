...
机器状态不是唯一交付界面。每次确认、候选生成、归档、还原、永久删除和迁移都必须在同一事务中刷新人类可读 Markdown：

- `memory/users/<channel>/<userId>/用户印象.md`
- `memory/groups/<channel>/<chatId>/群聊记忆.md`
- `memory/long-term/公共长期记忆.md`
- Agent Home 根目录 `记忆总索引.md`
- Agent Home 根目录 `记忆库说明.md` 中的受控“当前布局与状态”区块

规则：

- managed state 是唯一事实源，Markdown 是确定性生成投影，不形成第二事实源。
- `记忆总索引.md` 只列真实源文件、scope、已确认/候选/归档数量和更新时间，不复制具体事实正文。
- `记忆库说明.md` 的用户手写内容必须保留，只允许更新受控区块；只有 schema、目录或生命周期协议变化时才改写说明。
- 归档目录生成清晰中文 `记忆归档索引.md`，列出可恢复条目和来源，不包含删除审计或完整聊天证据。
- 机器 JSON、源 Markdown、总索引和归档索引必须使用同一事务 manifest 和 before-image；任一投影写入失败时全部回滚。
- `机器人身份.md`、`行为与安全规则.md`、`工具与环境.md` 不因普通记忆变化自动改写；它们仍只通过独立 Self-Maintenance 证据门禁更新。
...
- 任一生命周期动作后，源 Markdown、`记忆总索引.md` 和 `记忆归档索引.md` 与面板统计一致。
- 用户直接打开 Markdown 能区分已确认、候选和归档，不需要解码隐藏 JSON。
- 人类文档生成失败时 mutation 返回未完成，并恢复所有 before-image。
...
表情包不进入普通知识索引。表情包的持续学习、群聊/用户偏好覆盖、结构化 `avoidWhen`、版本和回滚由同目录《表情包语义进化与回滚设计》负责；两套系统共享“未确认不进入主事实、证据去重、归档可恢复和永久删除受控”的原则，但保持独立事实源。
