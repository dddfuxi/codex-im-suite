...
| `apps/control-panel` | C# 只维护无业务规则的薄 DTO、手工 wire 形状/媒体校验，并由反射测试逐字段核对 JSON Schema；React 只消费浏览器安全的共享 Contract。`speech-view-model.ts` 把 Runtime 细粒度状态投影为“听懂语音、语音回复、唱歌、克隆音色、消息渠道”五项用户能力，不复制 Provider 或模型业务规则。“能力 → 语音”页默认只展开开关、模型/音色选择和说话/歌声试听；渠道、Provider、限制、benchmark、组件诊断、完整音色库和授权参考音频导入按需折叠。所有保存、安装、导入和试听仍经 Runtime CLI，不直接修改状态文件或实现语音算法。 |
...
- Bridge 已经裁决并传入受限风格、歌词、语言和时长，因此 ACE-Step 生成固定走官方直接 DiT 路径：请求关闭 `thinking` 与 caption/language CoT，受管进程不初始化或常驻 0.6B LM，避免让辅助元数据生成占用主要延迟和显存。Supervisor 仍从 ACE-Step 官方 `get_gpu_config()` 获取硬件档位，并把 `offload_to_cpu_default` 与 `offload_dit_to_cpu_default` 映射到官方环境开关；8GB 档可在 VAE 解码前卸载 DiT，高显存档不会被写死为同一策略。ACE-Step 只负责生成跨平台稳定的 WAV，Runtime 以受管 FFmpeg 转成单声道 16kHz Ogg/Opus，再分别复验中间 WAV 与最终 Opus 的文件头、时长、大小和 Hash；中间 WAV 必须清理，模型进程在本次受管动作完成或失败后释放，不能与后续 Qwen TTS 常驻争抢 GPU。该优化不改变独立 Singing Host、模型 revision、歌词原文、输出校验或 benchmark 门禁，也不能作为绕过真实 RTX 3070 复测的依据。
