...
- Windows 下所有 live Bridge“重启”必须统一调用 `packages/bridge-runtime/scripts/daemon.ps1 restart`，由 `workflow-drain.ps1` 先等待 `running/retrying` Workflow 排空；超时必须取消重启并保留原进程，只有明确诊断为卡死后的人工恢复才可显式开启 force 逃生口。`config.env` 中的 drain/force 配置优先于父进程继承环境。新增或修改 drain 脚本时必须同时验证 live、portable、installer 复制链、发布指纹和控制面板复制安装漂移探针。
