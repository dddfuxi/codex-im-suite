# Release Notes

## 2026-04-18 14:13:04

- Subject: refresh panel; sync suite 2026-04-18 14:13:04
- Summary source: fallback-rule

### Preview

MCP changes: none

Control panel changes:
- M apps/control-panel/Program.cs

Other changes:
- M packages/bridge-core/src/lib/bridge/adapters/feishu-adapter.ts
- M packages/bridge-core/src/lib/bridge/bridge-manager.ts
- M packages/bridge-core/src/lib/bridge/host.ts
- M packages/bridge-core/src/lib/bridge/types.ts
- M packages/bridge-runtime/scripts/daemon.ps1
- M packages/bridge-runtime/scripts/supervisor-windows.ps1
- M packages/bridge-runtime/src/codex-provider.ts
- M packages/bridge-runtime/src/config.ts
- ... and 24 more

### Commit Body

Control panel changes:
- M apps/control-panel/Program.cs

Other suite changes:
- M packages/bridge-core/src/lib/bridge/adapters/feishu-adapter.ts
- M packages/bridge-core/src/lib/bridge/bridge-manager.ts
- M packages/bridge-core/src/lib/bridge/host.ts
- M packages/bridge-core/src/lib/bridge/types.ts
- M packages/bridge-runtime/scripts/daemon.ps1
- M packages/bridge-runtime/scripts/supervisor-windows.ps1
- M packages/bridge-runtime/src/codex-provider.ts
- M packages/bridge-runtime/src/config.ts
- M packages/bridge-runtime/src/main.ts
- M packages/bridge-runtime/src/store.ts
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/installer/payload/packages/bridge-runtime/scripts/daemon.ps1
- M release/installer/payload/packages/bridge-runtime/scripts/supervisor-windows.ps1
- M release/portable/CodexImSuiteControlPanel.exe
- M release/portable/packages/bridge-runtime/scripts/daemon.ps1
- M release/portable/packages/bridge-runtime/scripts/supervisor-windows.ps1
- ... and 12 more

## 2026-04-17 11:47:33

- Subject: sync suite 2026-04-17 11:47:33
- Summary source: fallback-rule

### Preview

MCP changes: none

Other changes:
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/installer/payload/scripts/publish-backup.ps1
- M release/portable/CodexImSuiteControlPanel.exe
- M release/portable/scripts/publish-backup.ps1
- ... and 1 more

### Commit Body

Other suite changes:
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/installer/payload/scripts/publish-backup.ps1
- M release/portable/CodexImSuiteControlPanel.exe
- M release/portable/scripts/publish-backup.ps1
- M scripts/publish-backup.ps1

## 2026-04-17 11:39:31

- Subject: refresh panel; sync suite 2026-04-17 11:39:31

### Preview

MCP changes: none

Control panel changes:
- M apps/control-panel/Program.cs

Other changes:
- M packages/bridge-runtime/config.env.example
- M packages/bridge-runtime/src/__tests__/config.test.ts
- M packages/bridge-runtime/src/__tests__/local-llm-router.test.ts
- M packages/bridge-runtime/src/config.ts
- M packages/bridge-runtime/src/local-llm-provider.ts
- M packages/bridge-runtime/src/local-llm-router.ts
- M packages/bridge-runtime/src/local-llm-status.ts
- M packages/bridge-runtime/src/main.ts
- ... and 10 more

### Commit Body

Control panel changes:
- M apps/control-panel/Program.cs

Other suite changes:
- M packages/bridge-runtime/config.env.example
- M packages/bridge-runtime/src/__tests__/config.test.ts
- M packages/bridge-runtime/src/__tests__/local-llm-router.test.ts
- M packages/bridge-runtime/src/config.ts
- M packages/bridge-runtime/src/local-llm-provider.ts
- M packages/bridge-runtime/src/local-llm-router.ts
- M packages/bridge-runtime/src/local-llm-status.ts
- M packages/bridge-runtime/src/main.ts
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/installer/payload/packages/bridge-runtime/config.env.example
- M release/portable/CodexImSuiteControlPanel.exe
- M release/portable/packages/bridge-runtime/config.env.example
- ?? packages/bridge-runtime/src/local-agent-provider.ts
- ?? packages/bridge-runtime/src/mcp-bridge.ts

## 2026-04-16 18:53:47

- Subject: refresh panel; sync suite 2026-04-16 18:53:47

### Preview

MCP changes: none

Control panel changes:
- M apps/control-panel/Program.cs

Other changes:
- M packages/bridge-runtime/config.env.example
- M packages/bridge-runtime/package.json
- M packages/bridge-runtime/src/__tests__/config.test.ts
- M packages/bridge-runtime/src/config.ts
- M packages/bridge-runtime/src/main.ts
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- ... and 20 more

### Commit Body

Control panel changes:
- M apps/control-panel/Program.cs

Other suite changes:
- M packages/bridge-runtime/config.env.example
- M packages/bridge-runtime/package.json
- M packages/bridge-runtime/src/__tests__/config.test.ts
- M packages/bridge-runtime/src/config.ts
- M packages/bridge-runtime/src/main.ts
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/installer/payload/packages/bridge-runtime/config.env.example
- M release/installer/payload/packages/bridge-runtime/package.json
- M release/portable/CodexImSuiteControlPanel.exe
- M release/portable/packages/bridge-runtime/config.env.example
- M release/portable/packages/bridge-runtime/package.json
- ?? packages/bridge-runtime/scripts/local-llm/
- ?? packages/bridge-runtime/scripts/run-tests.mjs
- ?? packages/bridge-runtime/src/__tests__/local-llm-router.test.ts
- ?? packages/bridge-runtime/src/local-llm-provider.ts
- ?? packages/bridge-runtime/src/local-llm-router.ts
- ... and 8 more

## 2026-04-16 13:21:19

- Subject: sync suite 2026-04-16 13:21:19

### Preview

MCP changes: none

Other changes:
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/portable/CodexImSuiteControlPanel.exe
- ?? config/skills.d/blender-mcp-glb-unity-pipeline.json
- ?? extensions/skills/blender-mcp-glb-unity-pipeline/
- ... and 2 more

### Commit Body

Other suite changes:
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/portable/CodexImSuiteControlPanel.exe
- ?? config/skills.d/blender-mcp-glb-unity-pipeline.json
- ?? extensions/skills/blender-mcp-glb-unity-pipeline/
- ?? release/installer/payload/config/skills.d/blender-mcp-glb-unity-pipeline.json
- ?? release/portable/config/skills.d/blender-mcp-glb-unity-pipeline.json

## 2026-04-16 13:12:54

- Subject: sync suite 2026-04-16 13:12:54

### Preview

MCP changes: none

Other changes:
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/portable/CodexImSuiteControlPanel.exe

### Commit Body

Other suite changes:
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/portable/CodexImSuiteControlPanel.exe

## 2026-04-16 13:09:52

- Subject: update MCPs; refresh panel; sync suite 2026-04-16 13:09:52

### Preview

MCP changes:
- M config/mcp.d/unity-mcp.json
- M release/installer/payload/config/mcp.d/unity-mcp.json
- M release/portable/config/mcp.d/unity-mcp.json
- ?? config/mcp.d/blender-mcp.json
- ?? extensions/blender/
- ?? release/installer/payload/config/mcp.d/blender-mcp.json
- ?? release/installer/payload/scripts/launch-blender-mcp.ps1
- ?? release/installer/payload/scripts/stop-blender-mcp.ps1
- ?? release/installer/payload/scripts/stop-unity-mcp.ps1
- ?? release/portable/config/mcp.d/blender-mcp.json
- ?? release/portable/scripts/launch-blender-mcp.ps1
- ?? release/portable/scripts/stop-blender-mcp.ps1
- ... and 4 more

Control panel changes:
- M apps/control-panel/Program.cs
- M packages/bridge-runtime/scripts/build-control-panel.ps1
- M release/installer/payload/packages/bridge-runtime/scripts/build-control-panel.ps1
- M release/installer/payload/scripts/sync-live-skill.ps1
- M release/portable/packages/bridge-runtime/scripts/build-control-panel.ps1
- M release/portable/scripts/sync-live-skill.ps1
- M scripts/sync-live-skill.ps1

Other changes:
- M packages/bridge-core/src/lib/bridge/bridge-manager.ts
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/installer/payload/scripts/bootstrap-suite.ps1
- M release/installer/payload/scripts/publish-backup.ps1
- ... and 8 more

### Commit Body

MCP changes:
- M config/mcp.d/unity-mcp.json
- M release/installer/payload/config/mcp.d/unity-mcp.json
- M release/portable/config/mcp.d/unity-mcp.json
- ?? config/mcp.d/blender-mcp.json
- ?? extensions/blender/
- ?? release/installer/payload/config/mcp.d/blender-mcp.json
- ?? release/installer/payload/scripts/launch-blender-mcp.ps1
- ?? release/installer/payload/scripts/stop-blender-mcp.ps1
- ?? release/installer/payload/scripts/stop-unity-mcp.ps1
- ?? release/portable/config/mcp.d/blender-mcp.json
- ?? release/portable/scripts/launch-blender-mcp.ps1
- ?? release/portable/scripts/stop-blender-mcp.ps1
- ?? release/portable/scripts/stop-unity-mcp.ps1
- ?? scripts/launch-blender-mcp.ps1
- ?? scripts/stop-blender-mcp.ps1
- ?? scripts/stop-unity-mcp.ps1

Control panel changes:
- M apps/control-panel/Program.cs
- M packages/bridge-runtime/scripts/build-control-panel.ps1
- M release/installer/payload/packages/bridge-runtime/scripts/build-control-panel.ps1
- M release/installer/payload/scripts/sync-live-skill.ps1
- M release/portable/packages/bridge-runtime/scripts/build-control-panel.ps1
- M release/portable/scripts/sync-live-skill.ps1
- M scripts/sync-live-skill.ps1

Other suite changes:
- M packages/bridge-core/src/lib/bridge/bridge-manager.ts
- M release/codex-im-suite-portable.zip
- M release/installer/CodexImSuiteInstaller.dll
- M release/installer/CodexImSuiteInstaller.exe
- M release/installer/CodexImSuiteInstaller.pdb
- M release/installer/payload/CodexImSuiteControlPanel.exe
- M release/installer/payload/scripts/bootstrap-suite.ps1
- M release/installer/payload/scripts/publish-backup.ps1
- M release/portable/CodexImSuiteControlPanel.exe
- M release/portable/scripts/bootstrap-suite.ps1
- M release/portable/scripts/publish-backup.ps1
- M scripts/bootstrap-suite.ps1
- M scripts/publish-backup.ps1
- ?? release/installer/payload/scripts/setup-blender-mcp.ps1
- ?? release/portable/scripts/setup-blender-mcp.ps1
- ?? scripts/setup-blender-mcp.ps1







