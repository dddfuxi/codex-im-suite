...
Unity 编辑器能力在控制面板中分为两个独立运行单元：`tool.aibridge` 通过项目注册表发现各 Unity 工程的 `.aibridge/cli`，只执行真实 `harness status` 检查并打开工程入口；`unityMCP` 仍属于 MCP manifest，可在面板中暂停/恢复。暂停 Unity MCP 会持久化 `enabled=false` 并停止托管 helper，AIBridge 不依赖 MCP、不会伪装成 MCP 或被 MCP 的自动启动逻辑带起。
...
flowchart TD
  ...
  Panel --> AIBridge[Unity AIBridge CLI]
  AIBridge --> UnityProjects[项目注册表中的 Unity 工程]
  McpLayer -. 可暂停 .-> UnityMcp
...
结构化判断是可替换的只读辅助链，不属于普通聊天模型路由。`packages/contracts/src/decision.ts` 与 `packages/contracts/schemas/decision.schema.json` 共同声明 `cti-decision-request/v1`、`cti-decision-result/v1` 和 `cti-decision-view/v1`，覆盖 `noul`（是/否概率）、`choice`（有限分类）和 `score`（评分及分布）。Core 通过可选 `DecisionProviderHost` 把已经裁决过的短状态与问题交给 Runtime；纯模式可以先经过独立的 `DecisionQuestionPlannerHost`。纯模式的 planner 以受限 classifier 先理解当前消息，生成 3–5 个完整的答案或解决方案候选，Core 再校验题型、长度、候选数量和敏感字段，并强制该阶段使用 `choice`；候选通过校验后才交给 Jev 评估。纯模式中自动识别的消息意图只作为辅助 evidence，不替代答案候选。结果只能作为只读 evidence，不能携带按钮回调、平台身份、路径、命令或凭据。

Runtime 的 `JevDecisionProvider` 是 OpenRouter Decisions API 的专用适配器，默认 endpoint 为 `https://openrouter.ai/api/alpha/decisions`、模型为 `typesafe/jev-1.13`，不会进入 Codex/Claude 普通聊天链。配置与受控 Panel Settings 描述符已经定义 `CTI_DECISION_PROVIDER=off|jev`、`CTI_DECISION_MODE=off|shadow|assist` 和 `CTI_DECISION_RESPONSE_MODE=off|explicit|auto`，并支持 endpoint、模型和超时；`CTI_JEV_API_KEY` 只在本机受控 `config.env` 中读取，控制面板只投影是否已设置，不展示密钥值。控制面板的结构化判断设置区可读取并保存这些非密钥配置，保存后需重启 Bridge 才生效。当前实现按 provider/key/非 off mode 装配 Jev Host；`/jev debug` 省略类型与 `auto` 继续使用 Core 的确定性组织，纯模式则先用 `ProviderDecisionQuestionPlannerHost` 以 classifier/严格 JSON 动态生成 3–5 个答案或解决方案候选，再把这条 `choice` 题交给 Jev，一次请求只评估规划后的候选。planner 或 Jev 关闭、缺少密钥、超时、取消、HTTP 错误或非法结果都不产生伪造概率；Feishu 卡片主展示是答案候选及概率分布，自动识别的意图只作为辅助信息，未进入判断入口的普通请求保持 Primary 链路。开发版变更要进入 live 时必须通过 `scripts/sync-live-skill.ps1` 构建并同步 suite/runtime 副本，再通过 `packages/bridge-runtime/scripts/daemon.ps1` 受控重启和运行态证据核对，不能只复制源码或凭启动命令退出码宣称生效。

```mermaid
flowchart LR
  Request[显式命令、后缀或聊天纯模式] --> Route{入口}
  Route -->|纯模式| Planner[受限 JSON planner]
  Route -->|debug/auto| Host[DecisionProviderHost]
  Planner --> Host
...
- `packages/bridge-core/src/lib/bridge/application/action-blocks.ts` 统一解析 `cti-reminder`、`cti-scheduled-task`、`cti-direct-message`、`cti-bridge-control` 和 `cti-artifact-promote`。该模块只做 fence 清理、JSON/字段归一化和安全字段过滤；`application/direct-message-policy.ts` 统一判断当前原文授权、可靠 outbound-ref 续办授权以及动作目标是否与本轮来源 `chatId` 完全一致，`bridge-manager` 只据此编排当前会话发送或跨会话确认。模型动作块中的用户 ID、角色、工作区和陌生目标字段不成为可信事实；只有目标 ID 与可信来源 `chatId` 精确相等时才可判为当前会话。计划任务的规范协议保持 `at / every / cron`：cron 兼容规范对象与显式 `CRON_TZ/TZ` 字符串；`at.datetime` 只有携带显式 RFC3339 偏移或独立 timezone 时才转为 `at.at`，其中偏移已经唯一确定执行时刻时内部统一使用 UTC、不会猜地区；`once + delay*` 以真实动作解析时刻转换为单次 UTC；`interval + every*` 转换为 `everyMs`，缺少锚点时使用同一解析时刻。所有兼容转换写 `NORMALIZED_SCHEDULED_TASK_FIELDS` 审计；裸 cron、无偏移本地时间、非正间隔和未知枚举继续失败关闭，并返回具体字段错误。`agent_turn` 缺省 `sessionMode` 时只允许归一为无工作区的 `isolated`，只有模型明确声明 `bound` 后才进入 Host 的可信工作区重建。`cti-direct-message` 兼容历史命名，但 `targetType=user` 与 `targetType=chat` 分别代表私聊和群投递；“在命名群里发”等明确群目标只进入 chat resolver，不能回落成用户私聊。原生回复本机器人已持久化结果的“现在测试一次 / 继续发送 / 确认”等短动作可继承进入裁决的授权；当前群直接受控发送，真正跨群仍保留 Owner、唯一目标解析和二次确认。Feishu adapter 先查本地绑定，再通过应用身份分页读取机器人真实所在群列表，仅按精确群名或忽略末尾“群/群聊/群组”的安全别名唯一解析；零结果或多结果失败关闭并要求准确群名/chat_id。
...
- `packages/bridge-core/src/lib/bridge/application/mentions.ts` 统一处理飞书 mention ID 字段兼容、字符串/name-only 目标提示、唤醒 alias、直接执行/流程叙述/诊断语义区分、多人交互中“明确开始 + 指定首位/回答者 + 每轮原生 mention 交接”的当前动作识别、显示名目标提取、泛称群体识别以及占位符和非地址化裸 `@` 清理。字符串目标只表达 Agent 选择，不成为可信身份；该模块不查询成员、不接受模型自造 ID，也不执行发送。群体称呼（如“大家”“群里的机器人”）始终只是正文语义，不进入成员解析或广播；“群里的小明”这类具体成员仍会进入当前群唯一校验。Manager 只对“用户本轮明确点名”与“Agent 以 `cti-final.mentions`、已验证结构化 mention 或自己正文中的裸 `@` 明确选择”同时命中的成员调用 resolver，绝不为触发平台动作自动向正文补写 `@名称`；ID 求交集、Owner/广播门禁和最终交付仍留在平台编排边界。
- `packages/bridge-core/src/lib/bridge/application/stickers.ts` 统一处理表情包发送意图、入站/出站 hint 隔离、标注与候选分析协议清理、置信度和具体语义门禁、仅允许本轮真实附件 fileKey 的一次性选择，以及“贴纸是否足以替代冗余伴随文字”的纯裁决。只有结构化 `explicitRequest=true` 才允许 sticker-only；社交短语或表面语气不能吞掉正文、任务结果、信息回答或失败说明。图片附件真实性、视觉模型调用、语义 revision 授权与写入、平台投递和真实 messageId 回执仍由 Manager、Sticker Host 与 Feishu adapter 共同完成，纯策略模块不能自行发送或确认语义。
...
- `/docs` 的飞书管理卡片由 `bridge-manager` 根据本地文档索引生成，Owner 可通过短期、绑定渠道/聊天/用户的回调进入二次确认；`FeishuAdapter.deleteDocument()` 只调用飞书云盘删除接口，远端成功后才由 `feishu-document-memory.ts` 按稳定索引 ID 同步删除 `index.json` 和 `DOCUMENT_GUIDE.md`。回调过期、越权、跨聊天或远端失败均失败关闭，不接受用户文本或模型直接提供任意文档 token。
- 飞书 lark-cli 用户授权由 Core 先记录真实 `user` API 缺权 scope，再在明确授权续办消息中恢复最近的单 scope 证据；Runtime 仍只接受官方 device-code、Owner、单精确 scope 和非阻塞 JSON 契约。`--domain`、`--recommend`、多 scope 与 bot 缺权冒充 user OAuth 均在治理层拒绝，授权成功后沿用原始任务文本恢复执行。
...
- `CollaborationEligibilityPolicy` 让简单聊天和确定性命令保持零协作 Specialist；复杂多源 evidence、低置信引用、已确认的记忆意图或复合架构任务才进入 Coordinator。记忆写入判断是独立的受限 JSON preflight：所有无附件的可分类文本回合都可得到 `write / ignore / clarify`，但 `ignore` 不产生写入、协作调度或用户可见状态。Coordinator 最多选择两个当前 Registry 中的 Specialist，父进程重新校验 Agent ID、能力、evidence 和预算。
- 轻量会话协调器只处理由 Context Broker 明确签发 `lightChatEligible=true` 的回合，严格输出 `reply / delegate / clarify`：`reply` 和 `clarify` 只形成无工具的候选正文并继续交给 Delivery，`delegate` 则把未经裁剪的原始回合交给 Primary。资格只依赖结构化焦点与真实 evidence 关系，不依赖寒暄、动作、响应速度或其他固定短语；只有主焦点唯一为当前消息且无需解析时才可签发。任何可靠回复、续办、未恢复的原生回复或歧义焦点都会直接进入 Primary，保留完整上下文、工具能力与证据门禁。文本也不会因“这张图/继续分析”等说法自动重新挂载最近媒体，可靠媒体只能来自当前附件、原生 reply 或受控历史 evidence。协调器超时、低置信或 JSON 无效时同样失败关闭到 Primary。
...
- `lark-cli` 用户身份按当前 Feishu sender 隔离到独立运行态配置目录，不能把 Owner 或其他 sender 的 user token 带给当前请求。bridge-core 会拦截模型的二维码文案和 `cti-final.images`，把原进度卡按红色“未完成”收口，再让 bridge-runtime 发送 Card 2.0 `open_url` 授权卡；同一 sender 与规范化 scope 只启动一个后台 `--device-code` 登录，重复任务合并，成功后自动逐个恢复原请求。授权卡不需要 `card.action.trigger`，也不要求新增网页应用能力；发送失败时只降级为官方 HTTPS 授权链接，不生成二维码。审计只记录 requestId、裁决和 scope，不保存 device code、授权 URL 或 token。
...
- Feishu @ 投递、事件订阅、回调、入站、通知送达等诊断文本，以及引用他人消息、一般玩法规则或未来流程说明里的 `@名字`，只作为受控 evidence prompt 交给 Agent。当前回合明确要求执行 @ 时，`application/mentions.ts` 只提取具体显示名；“大家”“各位”“群成员”“群里的机器人”等泛称群体始终只是可见问候内容，不进入成员 resolver 或广播。bridge-manager 随后调用 Feishu inspector 从当前群官方成员/机器人、本轮原生 mention、当前 sender 和受控历史 `<at>` 候选中做只读预检；Provider 只看到“唯一 / 歧义 / 未找到 / 查询失败”及显示名，不接收平台 ID。最终 Delivery 仅在用户本轮明确点名且 Agent 已用 `cti-final.mentions`、已验证结构化 mention 或自己正文中的裸 `@` 选择同一成员时，才重新核验并生成结构化 native mention；绝不自动在正文补写 `@名称` 反向触发平台动作。`cti-final.mentions` 的字符串或 name-only 对象会保留为不可信选择提示，最终仍必须经当前群平台 evidence 解析；bot-to-bot 回合只有由事件真实 sender app/open/user/union ID 与当前群官方候选唯一求交得到的本轮原生唤醒方，才能走专用回艾特链路。模型单方面其他名字、历史单独出现的名字、广播和关系称呼仍不能触发通知。出站结构化 mention 兼容 `userId/user_id/openId/open_id/id`（以及既有 `unionId/union_id`）；通用 `id` 与其他模型 ID 一样必须与本轮真实 evidence 精确求交集，不能直接成为平台身份。明确艾特请求缺少可信目标时，交付层只撤销原生 mention 动作并追加未投递说明，不覆盖正常答案和附件。`atAll/at_all` 属于广播动作；在建立独立 Owner 确认协议前默认拒绝。
...
12. Conversation Engine 会把 `tool_use_id -> tool input` 与对应 `tool_result` 配对；只有真实、成功且符合官方 URL 白名单的 `lark-cli auth login --scope ... --no-wait --json` 才生成飞书 CLI 用户授权 challenge。bridge-manager 在任何结果块或附件发送前优先接管该 challenge：没有真实 sender 身份时返回红色阻塞；有身份的成员调用 runtime broker，禁止继续发送模型声明的二维码、本地 QR 图片或“回复好了”文案。runtime 只在内存中保留当前有效期内的 device challenge，以 `sender userId + normalized scopes` 合并等待任务；每个 sender 的 Codex 工具回合显式注入其 `CTI_HOME/runtime/lark-cli-users/<hash>` 隔离配置目录，后台执行官方 `lark-cli auth login --device-code ...`，成功后自动重新入队原请求，失败或过期则发送不含秘密的红色未完成卡。任何成员都不能复用 Owner 或其他成员的用户 token，隔离目录缺失时失败关闭。
...
| `bridge-runtime` | 加载默认关闭的语音/歌声配置，维护唯一 Provider/Model/Voice Catalog，按显式路径、`CTI_HOME\runtime-deps` 与受控 PATH 规则解析依赖，管理单请求门禁和本机版本化 Sidecar，执行真实文件头/大小/时长/Hash 校验、媒体转换、ASR/TTS、独立 ACE-Step 1.5 loopback 客户端、Opus 产物验证与授权音色注册表。参考音色导入由唯一 live Runtime 对同一文件执行 ASR，确认文本不一致时失败关闭；复刻合成由 Qwen Base 自带 speaker encoder 对参考与生成音频计算余弦相似度，低于阈值的临时产物在 Runtime 内删除，不能进入 Core 投递。Qwen 模型只从固定 revision 的受管目录离线加载；ACE-Step 受管 Supervisor 只由 live Bridge 持有，固定绑定 `127.0.0.1`、单 Worker、可写 state root、只读模型根和每次启动随机临时 Bearer token。官方 `ACESTEP_CHECKPOINTS_DIR` 与固定 bootstrap 共同把 DiT/VAE/Embedding/LM 定位到已验 Hash 的模型根；外层下载回调只允许解析该根下已存在的单层普通目录，Handler 内部的 `_ensure_models_present` 与 `_sync_model_code_if_needed` 也同时禁用，不能把权重写入 state、在首轮补下载或反向改写受管组件。v2 文件集合若 marker 身份仍与当前 manifest 完全一致但少量文件损坏，受管安装器只下载损坏文件到同卷 stage、完成固定 Hash 后逐文件原子替换并支持失败回滚，不重下有效大权重，也不采纳身份不明的现有目录。token 只进入 ACE 子进程环境，由官方入口读取，不得进入 Windows CommandLine、状态文件、日志或控制面板协议；CLI 只经认证 mailbox 调用，不会刷新一次就启动第二个模型进程。普通生成必须命中当前模型、revision、硬件 Hash 与具体参考音色的 ready benchmark。 |
| `apps/control-panel` | C# 只维护无业务规则的薄 DTO、手工 wire 形状/媒体校验，并由反射测试逐字段核对 JSON Schema；React 只消费浏览器安全的共享 Contract。`speech-view-model.ts` 把 Runtime 细粒度状态投影为“听懂语音、语音回复、唱歌、克隆音色、消息渠道”五项用户能力，不复制 Provider 或模型业务规则。“能力 → 语音”页默认只展开开关、模型/音色选择和说话/歌声试听；渠道、Provider、限制、benchmark、组件诊断、完整音色库和授权参考音频导入按需折叠。试听回执通过精确媒体协议后立即进入内存播放器，不为只读试听重复执行全量状态刷新；WinForms 只对本机受信页面放行异步生成后的媒体播放并显式解除 WebView 静音，React 负责自动播放、滚动到播放器及手动播放失败提示。所有保存、安装、导入和试听仍经 Runtime CLI，不直接修改状态文件或实现语音算法。 |

普通语音执行不直接把自然语言词命中提升为 TTS 授权。`/voice off` 是绝对硬禁用，`/voice on` 是确定性会话开启；其余普通语音请求只能由 Primary 在 `cti-final.speech` 中返回受限 `voice_only` 呈现意图，再由 Bridge 结合真实入站语音 evidence、Runtime 策略、当前角色与风险裁决。SpeechHost 和当前激活音色属于同一 live Bridge Runtime，不按飞书聊天窗口复制；会话只保存 `/voice on|off` 呈现偏好。对无外部执行要求的明确“发语音 / 用某音色说话”请求，纯策略协议审查只要求 Primary 补齐 `speech.mode=voice_only`，不会直接授权合成；短请求在没有唯一可信原生回复/附件正文时还执行 36 个可见 Unicode 字符的默认口播预算，模型自行扩写的长故事必须在进入 TTS 前收短，用户明确正文和可信引用内容不被该默认预算截断。首次和最多一次修复均为 response-only，禁止 Shell/TTS/`edge-tts` 和 MP3/WAV 文件旁路。若用户明确要求“刚克隆的 / 复刻的 / 参考音色”，审查还必须补齐 `speech.voice_requirement=active_reference` 这一受控类别要求；模型仍不能指定 Profile ID，Runtime 发现当前激活的是预设音色或参考音色尚未通过相似度验收时失败关闭，绝不以 Serena 等预设音色替代。Primary 不得自行声称某个聊天窗口缺少语音能力，真实可用性、当前模型/音色、相似度门禁和原生投递只能由 Host 回执决定。模型不能选择 Provider、模型、音色 ID、路径、命令、`file_key` 或平台身份。当前消息语音可以成为本轮用户正文；原生回复指向的旧语音只作为带 relation/source ID 的上下文证据，不能把旧录音里的命令提升为本轮授权。明确演唱请求另走独立协议期待审查：该审查只要求 Primary 返回严格 `cti-final.singing`，本身不签发执行权限；唯一可靠的原生回复正文可作为“这个/that/it”的歌词来源。明确克隆/参考音色演唱还必须带 `singing.voice_requirement=active_reference`，Runtime 只从独立歌声音色配置解析参考 Profile，未激活或不兼容时失败关闭且不使用默认歌声冒充。无外部证据要求的演唱回合从首次调用起即为 response-only，不能访问 Shell/TTS/文件工具；缺协议时最多再做一次 response-only 修复，异常 Provider 仍返回工具副作用时直接失败关闭。

参考音色创建另有独立的确定性协议审查：明确克隆/复刻请求只允许进入 Bridge-owned `SpeechHost.importReferenceVoice`，Primary 不得用 Bash、路径或普通附件实现。Runtime 通过 `getReferenceVoiceImportPolicy()` 向 Core 暴露单一布尔策略；`CTI_SPEECH_OWNER_SELF_VOICE_AUTO_AUTHORIZATION=true` 时，唯一 Owner 本人的干净录音默认授权为仅限本机 TTS，Core 不再要求模型或用户重复复述权利、用途和单人录音三项声明，并从当前用户显式 `参考文本：…` 构造确定性动作覆盖模型字段。策略默认关闭；非 Owner、来源不明或替他人克隆仍失败关闭。无论策略是否开启，逐字参考文本都不能来自 ASR 猜测，Runtime 仍重新 ASR 并完成文本一致性、受管路径、Hash、质量与相似度门禁。

固定计划通知只有在创建协议明确携带 `taskAction.speech={mode:"voice_only"}` 时才进入主动语音链；该字段不接受 Provider、模型、音色或路径，也不从全局语音设置机械推导。Scheduler 触发后仍由 `deliverProactiveMessage` 复用同一 SpeechHost 身份、合成回执、SHA-256 上传复验、Runtime 释放和 Delivery 去重：成功只发送原生音频，失败只发送一次完整文字回退。绑定会话 `/voice off` 时保持文字；带互动卡、图片、文件、mention 或其他交互内容的主动投递不得转语音，`check_in` 继续以唯一可操作卡片交付。语音 outbound ref 明确记录“已生成 / 已发送 / 相似度已通过或预设音色不适用”，不能把生成、发送和相似度三种状态混成“克隆成功”。
...
- Sidecar 启动、普通媒体/ASR 请求与 TTS 模型生成使用分离的有界时限。TTS 由 Runtime 按 `provider + model + revision + hardware` 的 benchmark 热态耗时、输出时长、RTF 和本轮可见文本规模计算动态时限，再服从 `CTI_SPEECH_SYNTHESIS_TIMEOUT_MS` 绝对上限；首次没有 benchmark 时使用该受控上限，避免高质量模型冷启动或长文本被普通 90 秒请求阈值误杀。Sidecar 的语言无关切分器先保留句界，再把相邻短句按 300 字符上限装入同一段，避免每个句子分别触发一次高成本模型推理；真正超长句仍按软标点和硬上限有界切分。超时会终止本实例 Sidecar 进程树并返回 `tts_synthesis_timeout`，Core 仅把稳定阶段和“未生成/未发送/单次文字回退”写入审计，不记录正文、路径、模型目录或身份。
...
- 明确唱歌请求只允许 `cti-final.singing` 携带 `song_only`、可见音乐风格、完整歌词、语言、受限时长和可选 `active_reference` 类别要求；该 envelope 必须是纯文字回退加歌声指令，禁止同时夹带 `files/images`、普通 `speech`、Provider、模型、具体音色 ID/路径、URL、token、命令和平台身份。Runtime 使用独立 Singing Host 调用 ACE-Step `/release_task -> /query_result -> /v1/audio`，并把已验证 Ogg/Opus 交给飞书原生 `audio`；普通 TTS、`edge-tts`、MP3/WAV 文件和 `sendLocalFile` 不能冒充歌声或原生语音终态。
...
- 面板 CLI 与 live Bridge 之间的试听/benchmark mailbox 由 Runtime 独占：服务端长期轮询句柄可以 `unref` 以免阻止 Bridge 退出，但一次性客户端在收到归属匹配的响应或有界超时前必须持有活动等待句柄；普通语音试听与 benchmark 的外层等待使用同一独立 TTS 合成上限，不得再次退回普通 `requestTimeoutMs` 提前取消仍在生成的 live 请求。CLI 只能在主 Promise 终态后输出一条 JSON，禁止以退出码 0、空 stdout 把未执行伪装成成功。
- 受管歌声 Runtime 采用按需生命周期：周期状态刷新只核对固定 manifest、marker、组件状态和硬件绑定 benchmark，不得为健康探测启动或常驻 ACE-Step。实际歌声试听/benchmark 前先停止可按需重建的普通语音 Sidecar，避免 Qwen TTS 与 ACE-Step 同时占用本机 GPU；下一次 ASR/TTS 会通过原 Supervisor 重新建立 Sidecar。受管歌声请求的 Abort/超时必须同时终止本实例 ACE 子进程，不能只断开 HTTP 后让服务端继续 CPU/GPU 推理。Supervisor 只在旧 owner/child 都确认退出后清理上轮 `ace-step-host/temp`，并在本实例收到真实子进程终态后清理本轮 temp；清理逐项拒绝 symlink/junction 和未知节点，不能触碰同级 cache、模型、参考音色或最终交付目录。外部显式 loopback 歌声服务仍按真实 `/health` 探测，不受此受管生命周期替代。
- Bridge 已经裁决并传入受限风格、歌词、语言和时长，因此 ACE-Step 使用已固定的官方 5Hz LM 与 `thinking=true` 生成，不让 Runtime 以普通 TTS 或模型拼接歌词冒充唱歌。Supervisor 仍从 ACE-Step 官方 `get_gpu_config()` 获取硬件档位，并把 `offload_to_cpu_default` 与 `offload_dit_to_cpu_default` 映射到官方环境开关；8GB 档可在 VAE 解码前卸载 DiT，高显存档不会被写死为同一策略。ACE-Step 只负责生成跨平台稳定的 WAV，Runtime 以受管 FFmpeg 转成单声道 16kHz Ogg/Opus，再分别复验中间 WAV 与最终 Opus 的文件头、时长、大小和 Hash；中间 WAV 必须清理，模型进程在本次受管动作完成或失败后释放，不能与后续 Qwen TTS 常驻争抢 GPU。歌词验收默认使用独立 Sidecar；当显式启用 `fireredasr2_aed_wsl` 时，Runtime 仅向 F 盘受管 WSL 发行版传入已归一化 WAV 和本地 FireRed 模型目录，读取严格 JSON 转写回执后继续执行不低于 `0.8` 的 Unicode 歌词对齐与 Qwen Base 真实音色相似度门禁。FireRed 不能替换普通 ASR、不能下载模型、不能释放产物或向飞书投递。该优化不改变独立 Singing Host、模型 revision、歌词原文、输出校验或 benchmark 门禁，也不能作为绕过真实 RTX 3070 复测的依据。
...
- 模型 benchmark 按 `provider + model + revision + hardware hash` 存储；参考音色额外绑定 `voiceProfileId`，记录热态耗时、音频时长、RTF、峰值显存和 speaker encoder 相似度。Base 参考音色在当前组合未达到相似度阈值前不能激活或进入普通合成；面板可对每个 Profile 单独验收。benchmark 和参考文本核对都走同一单实例认证 mailbox，后者的路径只存在于短期本机请求文件，响应只返回文件 Hash 与 matched 状态，不向浏览器暴露路径、ASR 原文、参考音频或内部指标对象。
- 参考音色导入的媒体探针与实时媒体链复用同一受管 FFmpeg 发现规则：`ffmpeg_runtime` 可以同时提供 `ffmpeg/ffprobe`，也兼容独立 `ffprobe` 组件；不得因调用方遗漏组件别名而退回系统猜测路径或绕过真实文件头、时长和 Hash 门禁。Qwen Base 合成前还必须将当前受管原始参考音频转成单轮临时 PCM WAV；这只解决跨容器解码兼容性，原始文件、Hash、Profile ID 与授权元数据不变，临时副本不得进入注册表或长期目录。
- 参考音色注册表与受管音频是跨 Bridge 重启的长期事实源。重命名动作只接收 Profile ID 与新显示名，由 Runtime 在注册表中重新解析并拒绝 preset/catalog、空名、超长名和规范化后的重名；它只原子更新显示名与 revision，稳定 Profile ID、受管文件、授权证据和全部 benchmark 均保持不变。删除动作只接收 Profile ID，由 Runtime 在注册表中重新解析并拒绝 preset/catalog 身份；受管参考文件先在同卷移动为墓碑，再原子更新注册表并清理该 Profile 的全部 benchmark。当前说话音色命中时先回退到所选模型的兼容默认预设，找不到则清空；当前歌声音色命中时清空。React 只对 reference 卡片显示重命名和带影响说明的删除确认入口，C# 只转发白名单命令且审计不记录 Profile、名称、路径、转写或音频元数据。参考音色 `ready`、控制服务验收与真实 `RuntimeSpeechHost.synthesize()` 必须统一消费同模型、revision、硬件与 Profile 绑定且指标自洽的 speaker similarity 结论；模型热态耗时继续单独决定性能 benchmark 状态和超时估算，禁止因模型较慢把已经通过的音色身份误报为“相似度未验收”或阻断真实生成。

Runtime 统一暴露 `ready / optional_missing / blocked / error` 四态：`optional_missing` 表示可选能力未启用或组件未安装，不能升级为整个文字 Bridge 故障；显式坏路径、授权/安全校验或受管清单不完整进入 `blocked`，不能偷偷回退；运行时探测异常进入 `error`。TTS 模型 `state=ready` 只表示 Sidecar 已真实加载该模型，`benchmark.state` 与 `speech.output` capability 另外表达当前硬件性能是否通过；`tts_model_warm_benchmark_too_slow` 必须显示为“已加载但生成过慢”，不能合并成普通 ready。语音输入和输出默认都是 `false`。
...
- 模型选择器的可用目录由 `packages/bridge-runtime/src/codex-model-catalog.ts` 独占发现：官方来源通过隔离的 Codex app-server `model/list` 分页读取，OpenAI-compatible 来源读取受控 `/v1/models`，Ollama 读取 `/api/tags`；Runtime 只返回脱敏的 `cti-codex-model-catalog/v1` DTO。控制面板仅调用 `codex-model-catalog-cli.mjs` 并渲染下拉选项，不直接启动 app-server、读取 `CODEX_HOME`、API Key 或任意 provider URL。目录失败时保留“跟随来源默认模型”或已有扩展候选，不伪造模型或能力；隐藏项、重复项和未声明的输入模态会被过滤或保持未知。
...
- Bridge 生成的隔离 Codex Home 会剥离全局 `model_catalog_json`；桌面/STLLMTool 目录可能声明当前 Bridge 二进制不支持的 `max/ultra` 推理档位，不能让该外部配置阻断轻聊 app-server 预热并回退所有短消息到重型 Primary。
...
- `codex exec --json` 的 JSONL `turn.completed.usage` 会汇总进 `WorkflowRun.tokenUsage`；`WorkflowRun.execution.provider` 记录 adapter id，例如 `ollama` 或 `lmstudio`。Bridge Manager 对跨会话模型/工具回合默认只开放 2 个全局执行槽（`CTI_BRIDGE_MAX_CONCURRENT_TURNS` 可调至 1–8），同一会话仍保持 FIFO；官方/外部 Codex 单轮默认 15 分钟硬上限（`CTI_CODEX_TURN_TIMEOUT_MS` 可调，最多 30 分钟），本地 Codex CLI 子进程默认 5 分钟，`CTI_BRIDGE_PROCESSING_TIMEOUT_MS>0` 可覆盖。`CTI_BRIDGE_PROCESSING_TIMEOUT_MS<=0` 回到安全默认值，不再允许无限等待。断流恢复只按 offset 增量读取 rollout，每次最多读取 256 KiB，未发现 rollout 时扫描至少间隔 2 秒，待解析单行超过 2 MiB 则失败关闭，避免同步整文件 I/O 和无界缓冲。Windows 下超时或 abort 使用进程树终止，避免只杀 `cmd.exe` 而留下 `node codex.js` / `codex.exe` 僵住会话。
...
- `check_in` 的 callback 绑定真实任务、运行槽、投递 message ID、当前 chat 与原生 operator。`audience=chat_members` 时优先用群成员 API 复核，低风险成员 API 暂时失败才沿用原生 callback evidence 降级；`audience=owner` 只接受创建者。每人每轮最多一条记录，重复点击幂等返回；卡片刷新汇总人数，CLI/Host 历史保留 `checkInCount` 与真实参与者记录，Bridge 查询时再由当前渠道适配器解析显示名。普通回复、表情、通用投票和旧 reminder 的一次性完成按钮都不能写入该账本。
- “互动打卡 + 对比/统计/周期”请求先经 Runtime 读取当前 Actor 可见的本地打卡账本，再由当前渠道适配器按真实用户 ID 查询成员姓名，向 Provider 投影任务名称、运行时间、汇总 `checkInCount`、已验证参与者姓名和运行/投递状态；任务 ID、平台 ID、卡片内容和本机路径不得进入 Prompt。该路径明确不是飞书考勤、云文档或 Base，不得调用 `lark-cli` 或合并进飞书用户 OAuth；考勤类请求仍按外部资源权限链单独裁决。
- 同一计划槽使用稳定 `slotKey`，运行记录保存执行状态、投递状态和恢复信息。Bridge 重启后只恢复可证明安全的运行；未知副作用不自动重放。
- 同一调度 tick 先扫描并原子预留全部到期任务，再按 `CTI_SCHEDULED_TASKS_MAX_CONCURRENT_RUNS` 有界并发执行；无需模型的 `notify/check_in` 优先进入执行槽，避免慢 `agent_turn` 把整点打卡推迟到自身结束后。每轮运行额外记录 `dispatchDelayMs = queuedAt - scheduledFor`，旧账本缺少该可选字段时保持兼容。启动恢复和轮询会先按任务的 `misfirePolicy` 折叠过期窗口：`run_latest` 最多执行漏跑窗口内最新一轮，`skip` 丢弃积压轮次，超出 `maxLatenessMs` 的到期项直接推进到未来水位并记为 `misfire_skipped`；不会逐条追赶停机期间的历史发送。执行期间跨过的多个 interval 也会在收尾时一次推进到下一个未来槽位，成功执行但已过期的投递结果不会绕过同一门禁重发。
...
- Skill Registry / Lifecycle：扫描并合并 manifest、草稿、禁用项和正式安装项；通过官方 `skill-creator` / `skill-installer` 脚本执行创建、校验和安装，统一处理审批、审计、原子替换与回滚。官方脚本解析必须按受控 Codex Home 候选根查找（支持 `.system/<skill>/scripts` 与 `<skill>/scripts` 两种布局），缺失时返回已检查根路径的明确诊断，不能把隔离 Home 的路径缺口伪装成通用执行失败。
...
- Feishu CLI 用户授权 host：接收 bridge-core 从真实工具对提取的 `cti-feishu-cli-user-auth/v1` challenge，按当前真实 Feishu sender 的 `userId` 绑定授权请求，并在 `CTI_HOME/runtime/lark-cli-users/<hash>` 建立独立 `lark-cli` 配置目录；任何成员都只能为自己授权，禁止复用 Owner 或其他成员的用户 token。Windows/Codex 常见的 PowerShell `-Command '...'` 包装会让末尾 `--json` 后直接出现闭合引号，布尔 flag 解析按 shell 参数边界兼容引号、分号和右括号，但不接受相似长参数；challenge 仍须同一工具对、官方 URL、单一精确 scope 和成功结果共同验证。Card 2.0 按最小 scope 展示 `open_url` 按钮，后台 runner 用 argv 调用官方 `auth login --device-code`，并显式注入本人的隔离配置目录；同一 sender 与 scope 的并发任务共用一次轮询，成功自动恢复，拒绝/过期发送红色未完成结果。缺少真实 sender 或隔离配置时失败关闭，不回退共享 token；该 host 不持久化 device code、URL 或 token，也不替代 FeishuAdapter 的 bot 长连接。
...
- `bridge.restart` 先冻结旧 `PID / runId / startedAt`，再通过统一 120 秒有界 readiness 收口核对新进程、Supervisor/Service、`status.json`、`bridge-runtime-audit.json`、新启动时间、近实时心跳、空 `lastExitReason / lastUnhandledError` 和已启用渠道连接；该窗口覆盖可选语音 Runtime 与 Feishu WS 的现场冷启动，但不会无限等待。短命 PowerShell 包装器的超时或非零退出码只是诊断信号；完整新实例 evidence 成立时按成功返回，Workflow drain 明确延期/门禁缺失、身份未切换或审计不健康时失败关闭。命令错误只外发稳定诊断码，不再生成空尾巴的“Bridge restart 失败：”。
...
### 11.2 Owner 面板设置 Host

- `packages/contracts/src/panel-settings.ts` 与 `schemas/panel-settings.schema.json` 定义面板 wire 状态、设置描述、脱敏快照、变更和写入回执；React 直接消费共享 `PanelSettingsStateContract`，C# `SettingsSnapshot` 继续保持无业务规则的薄 DTO。
- `bridge-core/application/panel-settings-policy.ts` 只识别“受控设置对象 + 查询/修改意图”、构造 `cti-panel-settings-evidence/v1` 和用户可见脱敏列表；具体属性与值由 Primary 从 Runtime 当前目录解析，不按固定句子触发。
- `cti-panel-settings` 动作只允许 `action=update`、1–10 个稳定 `{key,value}` 标量和可选 `expectedVersion`。模型不能提交身份、env 名、命令、重启参数、API key/token 或路径来源。
- `bridge-runtime/panel-settings-host.ts` 独占目录、Owner 二次校验、枚举/数字/布尔/HTTP(S)/绝对路径校验、乐观版本、短期独占锁和保留注释/未知字段的 UTF-8 原子 env patch。只读派生项和敏感项只进入快照，不接受语音写入。
- 写入成功回执同时返回旧值、新值和当前完整脱敏快照；Core 只有收到 `written=true` 才调用固定 `BridgeControlHost.scheduleRestart()`。重启在当前回复交付后执行，Host 失败或写入失败不重启。
- 飞书语音沿用可信 `SpeechHost.transcribe()` 后的当前回合正文，因此文字与语音走同一 Owner、Policy、Host 和 Delivery，不维护第二套关键词权限入口。
# 计划任务隔离工作区与 Codex 信任检查

无绑定项目的计划任务使用短生命周期隔离沙箱。Provider 根据本轮 `workspacePlan.primaryWorkspace.projectId` 区分两类目录：隔离沙箱可跳过 Git 仓库信任检查，已注册项目仍必须经过 Codex 原生信任门禁。该判断集中在 `provider-workspace.ts`，同时由 SDK Provider 与本地 CLI Provider 复用。

## 飞书原生回复音频证据

Feishu adapter 恢复被回复的资源时，始终绑定原始 `message_id + file_key`。平台将音频作为 `file` transport 返回时，adapter 只根据已下载字节的真实文件头识别 WAV/Ogg/FLAC/MP3/M4A/WebM，并将可信绑定提升为 `resourceType=audio`；普通文件、文件路径、文件名和模型描述不能提升为音频。音色导入继续由 Core 的 Owner、逐字参考文本和 Runtime 音频质量/说话人相似度门禁收口。
### 参考音色克隆的文本门禁

用户触发格式与逐字验收分离：唯一 Bridge Owner 对可信原生录音直接提出克隆请求即可进入 Bridge-owned SpeechHost。Runtime 仍对同一受管音频执行二次 ASR，并仅在两次识别一致时登记；用户显式确认的文本继续支持原有 `user_confirmed` 路径。
# 选择卡参与者绑定

选择卡默认使用 `single_user + audience=initiator`，保持发起人专属。用户明确要求让当前群另一成员作答时，Provider 只能输出 `single_user + audience=participant`；Feishu adapter 在当前群成员名单中解析本轮原生 mention 或明确姓名，Bridge 再签发并持久化真实目标 ID。目标解析失败或同名时不发卡，回调仅允许已验证的目标成员点击；模型不得提供 participant ID、callback 或平台身份。
### TAPD MCP 凭据与项目上下文边界

TAPD 接入由 Runtime 的受管 MCP manifest 投影。`requiredEnvironment` 只检查凭据环境变量是否存在，token 不进入模型 Prompt 或持久化状态；`mcp-context/v1` 仅保存 manifest 声明的非敏感标识，并按当前渠道、聊天和用户作用域回注入 Primary。状态文件位于 `CTI_HOME/runtime`，采用统一 UTF-8 原子写入；受限 classifier/response-only 链路不会获得 TAPD 上下文。用户轮换凭据后必须重新配置受管 `config.env`，再显式同步 live 并重启 Bridge。
