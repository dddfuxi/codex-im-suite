...
运行时多 Agent 使用共享常驻 Worker 子进程池，不新增微服务。`Bridge` 仍是唯一平台收发者，`Primary Agent` 仍是唯一回复与工具执行者；`Coordinator / Context / Memory / Performance` 只返回严格 Schema 的只读 findings 和 Prompt section，不能返回工具动作、发送动作、权限、平台目标或写入声明。
