...
- 同一调度 tick 先扫描并原子预留全部到期任务，再按 `CTI_SCHEDULED_TASKS_MAX_CONCURRENT_RUNS` 有界并发执行；无需模型的 `notify/check_in` 优先进入执行槽，避免慢 `agent_turn` 把整点打卡推迟到自身结束后。每轮运行额外记录 `dispatchDelayMs = queuedAt - scheduledFor`，旧账本缺少该可选字段时保持兼容。
