...
计划任务不能把创建消息中的任意路径长期固化为未来工作区。执行上下文只保存受控绑定：
...
- `sourceSessionId` 必须来自当前 Bridge 会话，不接受模型编造。
- `workspaceMode=bound` 时，每次运行重新调用统一 `workspace-plan.ts`，解析该会话当前绑定的规范化工作区。
- 项目注册根和 `CTI_ALLOWED_WORKSPACE_ROOTS` 仍只是权限上界，不能自动变成挂载目录。
- 创建时消息里出现的其他绝对路径只能作为创建回合临时证据，不能沉淀进周期任务。
- 记忆库、运行态、上传缓存、日志和发布目录不能成为计划任务工作区。
- 如果未来执行时原绑定已经无效或越界，任务本次运行失败关闭并给出可观察诊断，不回退到进程 cwd、Unity 工程或其他最近工作区。
...
- 实际采用的规范化 workspaceId 和工作区裁决摘要；不保存额外目录中的文件内容。
- 脱敏错误、诊断摘要和下一次重试时间。
...
- `agent_turn` 每次执行前必须重新解析 `executionContext`，不得沿用创建回合已经过期的临时挂载。
...
- 非法表达式、非法时区和过去时间。
- 固定锚点在重启后保持不变。
- 绑定工作区在每轮重新解析，禁止回退到 cwd、记忆库、上传缓存或其他项目。
