...
Bridge 重启会清空 Provider 内存，因此还必须处理持久化在 channel binding 中的旧 `sdkSessionId`。不新增第二套 thread 状态库：runtime 启动时把普通 Codex 执行档案 fingerprint 纳入现有 `bridge_runtime_fingerprint`。模型来源、显式模型或普通任务推理强度发生变化后，`channel-router` 复用既有 runtime-change 逻辑清空 `sdkSessionId`，但保留当前 CodePilot session、工作区和聊天历史；下一轮直接创建 fresh Codex thread。配置未变化时，现有持久化 resume 行为保持不变。
...
- 保存并重启后，执行档案 fingerprint 变化会通过现有 bridge fingerprint 清空持久化 `sdkSessionId`，但不新建 CodePilot session 或丢失聊天历史。
