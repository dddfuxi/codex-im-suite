...
- [x] **Step 1: 写 official 模型与 thread fingerprint 失败测试**
...
- [x] **Step 4: 让配置变化在 Bridge 重启后清空持久化 SDK thread**
...
- [x] **Step 5: 更新旧测试对 thread map 和 official 行为的预期**
...
- [x] **Step 6: 运行 Provider、router 与类型检查**
...
- [x] **Step 1: 写 contract 和 workflow 持久化失败测试**
...
- [x] **Step 4: 扩展 runtime telemetry 采集**
...
- [x] **Step 5: 运行 contract、workflow 和边界测试**
...
- [x] **Step 1: 写 Web view-model 失败测试**
...
- [x] **Step 3: 实现 Web 纯函数并接入 main.tsx**
...
export function applyCodexSourceStrategy<T extends CodexStrategySettings>(current: T, strategy: AiStrategy): T {
  ...
  return { ...current, codexModelSource: strategy, codexRoutingMode: 'manual' };
}
```

来源切换只改变路由选择，保留未启用来源的模型、端点和密钥配置；Provider 必须以 `codexModelSource` 为准，不能因为残留 Base URL 抢占 official/local 来源。
...
- [x] **Step 3: 更新 README、开发日志和 AGENTS**
...
- [x] **Step 4: 通过实际控制面板应用一组可验证配置**

在用户实际访问的 live 面板：

1. 选择 official。
2. 模型填写当前账号已知可用的模型；若没有第二个可验证模型，使用当前有效模型。
3. 推理强度选择非默认值；实际使用 `xhigh`。
4. 点击“保存并重启 Bridge”。
5. 确认页面显示“Bridge 已加载”，而不是“服务端已确认”。
6. 验证 `config.env` 中模型、派生的 `CTI_CODEX_PASS_MODEL=true` 和推理强度一致。

- [x] **Step 5: 从真实 Feishu 入口验证普通 Agent 回合**

验收时未额外冒充用户发送私聊消息，而是复用 Bridge 重启后真实自动恢复的 Feishu 普通 Agent 工作流，避免干扰用户正在执行的 Unity 任务。实时进程证据确认：

```text
codex.exe exec --model gpt-5.6-sol
model_reasoning_effort="xhigh"
--sandbox danger-full-access
--cd F:\unity\ST4
```

该入口证明普通任务实际使用显式 official 模型和 xhigh；同 fingerprint resume、profile 变化 fresh 和 Workflow 参数持久化继续由 Provider / workflow 自动测试覆盖。没有把恢复中的业务 run 伪装成独立私聊验收消息。

Workflow 的 `submittedModel / submittedReasoningEffort / parameterEvidence / threadMode` 持久化由 workflow 14/14 和 Provider 42/42 自动测试验证；本次恢复中的旧 run 没有伪造这些新字段。

- [x] **Step 6: 验证受限回合覆盖与错误不静默降级**
...
实际采用最后一种方案：classifier / response-only 的 `low + restricted_interaction`、不存在模型不回落来源默认均由自动测试覆盖；未在 live 写入不存在模型，以免中断当前 Feishu/Unity 工作流。
...
git push origin codex/effective-model-settings
```
...
git rev-list --left-right --count HEAD...origin/codex/effective-model-settings
```

Expected: 工作区干净，ahead/behind 为 `0 0`。

### Task 9: Runtime 模型目录自动发现（2026-09-20）

- [x] Runtime 独占官方 app-server `model/list` 分页、隐藏项过滤和脱敏 DTO。
- [x] OpenAI-compatible 来源读取受控 `/v1/models`，Ollama 读取 `/api/tags`；地址与凭据只从 Runtime 配置读取。
- [x] 控制面板改为调用 `codex-model-catalog-cli.mjs` 并渲染模型下拉框；删除面板直启 app-server 的旧实现。
- [x] 补充 Contracts、Runtime、C# Gateway 和 Web 定向测试；已同步 live 并受控重启 Bridge，live 目录回执实际包含 `gpt-6-astra`，未把目录结果扩展成服务端实际调用成功。

### Task 10: 模型自我介绍使用本轮 Runtime 证据（2026-09-20）

- [x] 复现并确认旧 GPT-5.4 自报来自会话/飞书近邻历史，而不是 GPT-6 或 Grok 的实际提交路由。
- [x] 在 Agent Kernel 注册 `agent_kernel.runtime_model_identity`，约束历史自报、代理厂商和缺失值处理。
- [x] 普通 Codex、app-server 轻聊和本地 Codex CLI 从各自真实提交模型注入受控身份段，并覆盖 GPT-6、Grok、未知模型和异步配置漂移回归。
- [x] 完成 Core/Runtime 类型检查、定向 Provider 测试、依赖边界和人类文档门禁；已执行 suite → live 构建与同步并受控重启 Bridge，开发/live `daemon.mjs` 与 Core 模型身份策略产物哈希一致，Bridge 状态 `running`、Feishu WS=`connected`。真实飞书新消息复测仍待用户触发。
