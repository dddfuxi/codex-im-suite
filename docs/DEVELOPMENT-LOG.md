...
- 2026-07-21 Codex 模型与推理强度真实生效：根因是控制面板切到 official 时主动清空模型、关闭 `codexPassModel` 并重置推理强度，Provider 又对 official profile 强制忽略 `CTI_CODEX_MODEL`。现新增统一 `CodexExecutionProfile`，官方/外部/本地来源按“模型为空使用来源默认、非空显式提交”解析，普通任务把所选推理强度写入 SDK，classifier/response-only 固定 low 并记录覆盖原因。模型来源、模型、推理或端点身份变化后不再复用不兼容 thread；执行档案 fingerprint 纳入 bridge fingerprint，重启后只清空旧 `sdkSessionId`，保留聊天 session 和历史。Workflow 与面板区分请求值、SDK 提交值和服务端回报值，SDK 0.132.0 未提供服务端模型回报时只显示“已提交给 Codex”。TDD 已覆盖执行档案 5/5、Codex Provider 42/42、router 11/11、contracts 11/11、workflow 14/14、Web 29/29、控制面板 100/100，并通过 TypeScript、Web、C# 构建与依赖边界门禁；live 同步和真实飞书回合验收待最终阶段完成。
